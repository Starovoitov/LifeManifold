...
Reader-facing contrast labels are \textbf{H1}--\textbf{H5} (Table~\ref{tab:role-exp}). Prompt placeholders and threshold gates are mechanisms \emph{inside} a role, not alternate role names. Legacy protocol strings (\texttt{RQ*}, \texttt{H2}, \texttt{F-RQ*}, \texttt{F-B4-*}) appear only in the appendix crosswalk (Table~\ref{tab:families}).
...
\textbf{How to read labels.} The main text uses only \textbf{H1}--\textbf{H5} (Table~\ref{tab:role-exp}). H1 and H2 are the primary before-/after-generation contrasts; H3 is the LLM stacking interaction (exploratory: compose-gate blocked); H4 is the continuous-relaxation pyribs reference; H5 is the second-domain transfer check. Shared-checkpoint offline validity is a gate, not an H-label. Locked historical family IDs live only in appendix Table~\ref{tab:families}.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    Both under LLM & \texttt{hints} vs.\ \texttt{filter} & H3 (interaction) & exploratory (compose-gate blocked) \\
    Continuous-relaxation reference & hints vs.\ cma\_me / cma\_mae & H4 & protocol control (family FAIL) \\
    Second-domain transfer & dungeon AUC factorial & H5 & PARTIAL (H2 ports; LLM does not) \\
    \bottomrule
  \end{tabular}%
  }
\end{table}
...
\begin{enumerate}
  ...
  \item[\textbf{H5}] \textbf{Second-domain transfer.} Does after-generation sample-efficiency (and/or bundled LLM lift) port to a dungeon factorial at matched evaluation AUC?
\end{enumerate}
...
Section~\ref{sec:method} states the LLM-guided MAP-Elites loop and the before-/after-generation surrogate roles (Figure~\ref{fig:pipeline}), then summarizes the measurement substrate. Section~\ref{sec:protocol} states the confirmatory design. Section~\ref{sec:results} follows H1 $\rightarrow$ H2 $\rightarrow$ H3, then H4 (pyribs reference), multi-provider, cost, and H5 (second-domain transfer). Section~\ref{sec:discussion} separates architecture from payoff. Domain-knob tables and arm maps sit in the appendix where possible.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}llp{3.2cm}l@{}}
    ...
    \textbf{This work} & Offline MLP/GBM (shared) & \texttt{WorldSpec} mutator; roles compared & Optional (H2) \\
    \bottomrule
  \end{tabular}%
  }
\end{table}
...
The locked compose-gate check for LLM+filter non-inferiority remains \textbf{blocked}: live proposal replay at threshold~0.45 yields divergent skip rate $d=0.528$ (required $\le 0.05$). H3 is therefore \textbf{descriptive / exploratory by design} in this version---not a deferred footnote to a confirmatory claim. H2 reports matched \texttt{genetic\_me\_uniform} vs.\ \texttt{genetic\_me\_filter} as the primary filter evidence (\S\ref{sec:h2}).
...
Confirmatory Holm families are locked in the dated protocol files (\S\ref{sec:pins}). In plain terms: the bundled stub$\rightarrow$hints and hints$\rightarrow$vanilla stack contrasts \textbf{pass}; the pyribs reference family vs.\ CMA-ME/MAE \textbf{fails} (hints do not beat both arms; CMA-ME leads under continuous encoding); the dungeon AUC family is \textbf{partial} (H2 filter ports; LLM contrasts do not). Matched-policy H1 is exploratory and is \emph{not} the bundled stub vs.\ hints family. H3 (LLM+filter interaction) is \emph{not} among confirmatory Holm families---compose-gate blocked; descriptive only. Repository family ID strings and Holm widths $m$ are listed only in appendix Table~\ref{tab:families}.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}llp{6.4cm}@{}}
    ...
    Hold-out (shared-checkpoint validity) & $R^2{=}0.76$ $[0.70,0.82]$, MAE$_{\text{fit}}{=}0.007$ $[0.005,0.008]$ & $n{=}7{,}023$ hold-out; bootstrap 95\% CI; \texttt{hints\_ok} PASS \\
...
\textbf{Scientific shared-checkpoint validity (surrogate validity):} offline hold-out $R^2$ and MAE on recomposed fitness from checkpoint summaries, with bootstrap 95\% CIs over hold-out rows ($B{=}10{,}000$); supports but does not replace simulator endpoints.
...
\begin{table}[!htbp]
  \centering
  \caption{Bundled stack: paired hints $-$ stub (grid, $n=10$). $PS$ is Eq.~\eqref{eq:ps}. Holm \textbf{PASS} is a \emph{stack} outcome; matched H1 is Table~\ref{tab:decomposition}.}
  \label{tab:rq1}
  \fitcenter{%
...
\subsection{H3 --- Interaction check (exploratory; compose-gate blocked)}
\label{sec:h3}

After H1 (before-generation) and H2 (after-generation), H3 asks whether adding the same after-generation gate to the LLM$+$hint stack helps further. Confirmatory non-inferiority remains blocked ($d=0.528 > 0.05$ required), so we report levels only. On the bundled LLM tier, \texttt{filter} vs.\ \texttt{hints} is $-1.14$~pp coverage at fixed iterations ($59.27\%$ vs.\ $60.41\%$). Stacking both slots is not a simple sum of H1$+$H2. Primary after-generation evidence stays H2 (matched genetic arms; \S\ref{sec:h2}). The dungeon factorial gives partial transfer evidence for filter AUC under Holm (\S\ref{sec:results}).
...
\begin{table}[!htbp]
  \centering
  \caption{Pyribs continuous-relaxation reference: Holm step-down ($m=4$, paired hints $-$ arm, one-sided greater). Protocol family F-RQ-ceiling; not H2. $PS$ is Eq.~\eqref{eq:ps}.}
...
\subsection{H5 --- Second-domain after-generation check (dungeon; not cross-domain LLM QD)}
\label{sec:h5}
...
\begin{itemize}
 ...
 Offline $R^2_{\mathrm{fitness}}{=}0.76$ [95\% CI $0.70{,}0.82$] (shared-checkpoint validity) and low false-accept mass among evaluated proposals ($12.7\%$ with true fitness $<0.45$; MAE $0.066$) suggest the checkpoint is accurate enough to support a hard threshold. The same predictor feeds H1 and yields a flat matched contrast, which suggests that \emph{attach role}---gate vs.\ advice---drives the asymmetry more than predictor quality alone.
...
Under continuous-relaxation encoding, pyribs CMA-ME leads hints by $\Delta\text{cov} \approx -13$~pp (pyribs-reference family FAIL); Bernoulli-decode CMA-ME leads by $\sim$11~pp and native bit-flip CMA-ME by $\sim$5~pp---both trail \texttt{rint} (0/10 seeds for native). CMA-MAE is near parity on coverage; the sole ceiling-family reject is a small fitness edge vs.\ CMA-MAE ($\Delta\text{fit}{=}+0.010$). Under LLM, stacking the after-generation gate with hints does not improve fixed-iteration coverage (about $-1$~pp), and formal H3 stays blocked by compose-gate divergence. On the dungeon domain, genetic MAP-Elites leads all LLM arms by $\sim$19~pp coverage, while after-generation sample-efficiency still ports---suggesting that filter engineering transfers more readily than bundled LLM QD lift.
...
\textbf{arXiv v1 scope (preprint vs.\ journal extension).} The preprint reports all frozen CA confirmatory families (v2/v3) unchanged and the dungeon confirmatory AUC family on the \emph{5k five-arm matrix} (\texttt{q1-v4-dungeon-rerun}, $n{=}10$; \textbf{PARTIAL}). Supplementary dungeon evidence adds only the CPU H2 pair at CA-matched proposal budget (\texttt{q1-v4-dungeon-full-cpu}; Table~\ref{tab:b4-fullcpu}). Full-budget LLM dungeon arms, a \texttt{random} MAP-Elites baseline @ 32.5k, one dungeon ablation family, and pbCMA-style discrete CMA tuning are deferred to a post--arXiv journal revision under a dated protocol amendment. Bernoulli-decode (package~C4) and native bit-flip \texttt{DiscreteCMAEmitter} supplementary arms ($n{=}10$ each) are reported in \S\ref{sec:h4}.

\textbf{Multi-provider and dungeon scope.} Extra LLM providers complete the confirmatory per-provider \emph{bundled} contrast at $n{=}10$ (Holm \emph{within each provider}---never a pooled cross-LLM family). That replication does \emph{not} re-run matched-policy H1 (\texttt{stub\_uniform} vs.\ \texttt{hints}); the primary matched null remains \texttt{qwen-turbo} $n{=}10$, with only an exploratory weak-model matched pilot ($n{=}3$) on a third endpoint. Absolute coverage under \texttt{gpt-4o-mini} lags stronger models even when bundled $\Delta$ direction is stable. Dungeon timing-to-threshold metrics were not locked before the first feasibility pilot and are exploratory; confirmatory dungeon endpoints are AUC at matched evaluation budget under the v4 protocol on the 5k five-arm matrix.
...
\textbf{Pipelines and instrumentation.} The CMA ceiling contrast compares complete QD pipelines (homogeneous five-emitter CMA pools vs.\ heterogeneous MAP-Elites+LLM), not a factorial decomposition of emitters. A post-hoc decode/cold-start ablation (\texttt{q1-cma-encoding-ablation}, seeds~0--4), Bernoulli-decode CMA-ME (package~C4, $n{=}10$), and native bit-flip CMA-ME ($n{=}10$; \S\ref{sec:h4}) show the hints$\ll$CMA gap survives alternate decodes ($+10.8$~pp Bernoulli vs.\ hints; $+5.2$~pp native vs.\ hints; $\geq +8.8$~pp on the weakest encoding-ablation arm) but native discrete trails \texttt{rint} on every seed ($-7.8$~pp mean). Matched evaluation budget is not matched dollar cost (\S\ref{sec:cost}). Floating model aliases and API rate limits constrain scheduling; we window-pin endpoints and log \texttt{llm\_spec\_hash}.
...
\noindent\textit{Readout:} $11/20$ paper-wide rejects. Stack (legacy F-RQ1), floor (F-RQ0), multi-provider (F-RQ1g), and dungeon H2 AUC transfers still reject after the global correction; H4 ceiling and the non-transfer dungeon contrasts remain non-rejects. Artifact: \texttt{artifacts/experiments/paper\_wide\_holm\_robustness.json}.

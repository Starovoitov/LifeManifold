...
\begin{abstract}
Attachment-point decomposition of a shared surrogate in MAP-Elites reveals \emph{asymmetric} value: after-generation evaluation gating can help; matched-policy before-generation prompt-side advice does not. Quality-diversity (QD) and surrogate-assisted evolutionary algorithms (SAEAs) illuminate behaviour archives under expensive black-box evaluation. Prior LLM-guided QD work often compares complete pipelines; we treat an LLM as one MAP-Elites variation operator and ask where the same surrogate attaches in the propose$\rightarrow$evaluate loop, holding checkpoint, archive, budget, and emitters fixed. On our controlled evaluator, after-generation gating improves coverage \emph{per simulation} by about $+3.7$~pp at mid-budget---an evaluation-count win, not a terminal-coverage or wall-clock win on this cheap testbed. At matched archive policy, prompt-side scalars stay within $\pm 2$~pp of stub constants (H1; $n{=}10$ on \texttt{qwen-turbo}); larger historical ``hints'' lifts confound surrogate use with target-selection policy. Continuous-relaxation CMA-ME leads the LLM stack on coverage (${\sim}$13~pp), but that gap is an encoding-asymmetric upper-bound reference---not a fair discrete-vs-discrete duel---and magnitudes remain testbed-specific. The contribution is surrogate \emph{model management} in MAP-Elites / hybrid EC emitters---not NLP systems or software engineering.
\end{abstract}
...
Quality-diversity (QD) is an evolutionary-computation (EC) family that maintains archives of solutions that are both high-performing and behaviourally diverse. MAP-Elites assigns each candidate to a behavioural niche and keeps the best elite per cell~\cite{mouret2015illuminating}. Every candidate still needs a black-box evaluation; when that roll-out is costly, evaluation count dominates wall time and surrogate-assisted illumination---a QD instance of classical SAEA model management~\cite{jin2005fitness}---becomes attractive~\cite{gaier2018sail}.

This paper's scientific object is that EC loop, not an LLM product and not software engineering. LLMs appear here only as one optional \emph{variation operator} that edits structured genotypes inside MAP-Elites (alongside genetic and random emitters). An LLM-guided stack still couples emitters, a simulator that buys ground truth, and an archive; the LLM does not observe simulator outcomes unless the loop feeds feedback into the prompt. A surrogate is a natural place to inject that feedback---but \emph{where} it attaches is usually confounded with the rest of the stack. The question we isolate is the EC one: before-generation soft advice vs.\ after-generation hard evaluation gating under a fixed MAP-Elites schedule.
...
\begin{enumerate}
  \item \textbf{EC attachment-point decomposition.} We isolate surrogate \emph{model management} inside MAP-Elites: same checkpoint, before vs.\ after generation, rest of the stack fixed (Figure~\ref{fig:pipeline}; \S\ref{sec:decomposition}--\ref{sec:h2})---an SAEA-style question for hybrid EC emitters, not an LLM-systems bake-off.
  \item \textbf{Bundled ``hints'' lift is mostly not hint content.} A locked stub$\rightarrow$hints contrast looks large ($\approx{+}15$~pp) but confounds archive policy with surrogate use. At matched \texttt{uniform\_frontier} on \texttt{qwen-turbo}, policy alone recovers $\approx{+}14.8$~pp; live prompt scalars vs.\ stub constants stay within $\pm 2$~pp (H1; $n{=}10$; post-hoc TOST). That matched null is \emph{not} multi-provider: DeepSeek / \texttt{gpt-4o-mini} replicate only the bundled (confounded) stack at $n{=}10$; a matched \texttt{gpt-4o-mini} pilot is $n{=}3$ exploratory only.
  \item \textbf{Only after-generation attachment (H2) improves the evaluation-count axis.} Threshold gating without an LLM gains $\sim$3.7~pp coverage per simulation at 15k--20k evaluations and skips 33.5\% of sims (Figure~\ref{fig:acquisition}). On this cheap evaluator that is \emph{not} a practical deploy win: terminal coverage $-0.83$~pp and wall clock $\sim$20 vs.\ $\sim$7~min; the $\sim$7$\times$ slower-sim wall break-even is extrapolated, not measured (\S\ref{sec:cost}).
  \item \textbf{Efficiency ranking is not LLM-favouring.} On pure archive coverage / cost, the LLM stack is not the winner here: continuous-relaxation CMA-ME leads hints by $\sim$13~pp (H4; upper-bound reference), native bit-flip CMA-ME by $\sim$5~pp, and matched-policy genetic ME is within $\approx{+}1.4$~pp of LLM+\texttt{hints} at $\sim$10$\times$ lower wall time (\S\ref{sec:h4}, \S\ref{sec:cost}). On dungeon/maze, genetic MAP-Elites leads all LLM arms. This paper studies surrogate \emph{placement} inside LLM-guided loops, not a claim that LLM emitters dominate non-LLM QD on metrics. ``Human-readable'' LLM edits remain a qualitative niche hypothesis (no user study).
  \item \textbf{Stacking under LLM (H3) is exploratory.} The locked production filter fails compose-gate skip agreement; we localize the mismatch (\S\ref{sec:h3}; appendix~\S\ref{sec:h3-appendix}) and defer a gray-zone repair.
  \item \textbf{Placement ports more readily than bundled LLM lift.} After-generation AUC transfers on dungeon and on maze CPU H2 ($n{=}10$); an interim maze five-arm slice ($n{=}5$) is sign-consistent but underpowered for Holm. Genetic MAP-Elites still leads LLM arms there (H5).
\end{enumerate}
...
We study an LLM-guided MAP-Elites propose$\rightarrow$evaluate loop with one shared surrogate and two attach points (Figure~\ref{fig:pipeline}). The evaluator is a measurement substrate only (\S\ref{sec:substrate}). Archive fitness always comes from the simulator---the surrogate never replaces ground truth for insertion.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    \midrule
    Before-generation & \texttt{stub\_uniform} vs.\ \texttt{hints} & H1 & exploratory (TOST$\pm$2~pp; \texttt{qwen-turbo}) \\
    After-generation & \texttt{genetic\_me\_uniform} vs.\ \texttt{genetic\_me\_filter} & H2 & eval-count gain (not wall) \\
...
\textbf{Power and non-rejection.} Formal paired-$t$ MDE / TOST power is reported only for matched-policy H1 (exploratory equivalence at $|\Delta\mathrm{cov}|\le 2$~pp; Table~\ref{tab:decomposition}). Elsewhere, a Holm non-reject is \emph{failure to reject}, not evidence of a true null or equivalence. Where useful for the reader, we quote retrospective MDEs from observed paired SDs (appendix~\S\ref{sec:stats-appendix}): they do not change locked pass/fail rules. Large wrong-direction effects (e.g.\ H4 CMA-ME leading hints by $\sim$13~pp) are not underpowered nulls; compose-gate-blocked H3 is deferred confirmatory design, not a powered interaction test.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}llp{4.2cm}p{3.6cm}@{}}
    ...
    2026-07-28 & Hold-out compose gate aligned to runtime $0.95$; gray-zone H3 path specified and \emph{deferred} & Locked production H3 remains blocked/exploratory; no re-Holm of H2/H4/H5 on new runs \\
    \bottomrule
  \end{tabular}%
  }
\end{table}
...
Confirmatory Holm contrast sets are locked in the dated protocol files (\S\ref{sec:pins}). In plain terms: the bundled stub$\rightarrow$hints and hints$\rightarrow$vanilla stack contrasts \textbf{pass}; the H4 pyribs reference vs.\ CMA-ME/MAE \textbf{does not} (hints do not beat both arms; CMA-ME leads under continuous encoding); the H5 dungeon AUC set is \textbf{partial} (H2 filter ports; LLM contrasts do not); an interim maze five-arm slice ($n{=}5$) is sign-consistent with that partial pattern but underpowered for Holm $m{=}8$, while a complete maze CPU H2 pair ($n{=}10$) already rejects after Holm. Matched-policy H1 is exploratory and is \emph{not} the bundled stub vs.\ hints contrast. H3 (LLM+filter interaction) is \emph{not} among confirmatory Holm sets---the locked production filter fails compose-gate agreement; the mechanism is diagnosed offline; a gray-zone repair is deferred. Holm widths $m$ and outcomes: appendix Table~\ref{tab:families}.
...
\textbf{Primary endpoints:} terminal coverage (\%) and mean best elite fitness on the collapsed archive. Confirmatory inference uses paired seeds ($n{=}10$ unless noted), one-sided Wilcoxon signed-rank tests, and Holm within each pre-specified contrast set (\S\ref{sec:protocol}). Tables may also report a paired probability of superiority $\mathrm{PPS}$ (Eq.~\eqref{eq:pps}; appendix~\S\ref{sec:stats-appendix}): the seed-wise proportion of favorable paired differences (ties as half)---\emph{not} independent-sample Vargha--Delaney $A_{12}$~\cite{vargha2000new}, and \emph{not} a propensity score. $\mathrm{PPS}$ is a \emph{descriptive} common-language companion only---not a confirmatory gate. Matched-policy H1 additionally uses a post-hoc TOST at $|\Delta\mathrm{cov}|\le 2$~pp (details in appendix; result in \S\ref{sec:decomposition}). Exploratory / pilot arms report mean $\Delta$, sign counts, and bootstrap CIs and are labeled as such. Surrogate hold-out $R^2$/MAE/NMAE support but do not replace simulator endpoints (\S\ref{sec:checkpoint}).
...
\begin{table}[!htbp]
  \centering
  \caption{Bundled stack: paired hints $-$ stub (grid, $n=10$). $\mathrm{PPS}$ (Eq.~\eqref{eq:pps}) is paired favorable-pair rate---not $A_{12}$, not a propensity score, and not a Holm / $d_z$ gate. Holm passes as a \emph{stack} outcome; matched H1 is Table~\ref{tab:decomposition}.}
  \label{tab:rq1}
  \fitcenter{%
...
\textbf{Threshold sensitivity.} A matched-policy sweep (Table~\ref{tab:h2-tau-sweep}) varies $\tau\in\{0.35,0.45,0.55\}$ at fixed \texttt{uniform\_frontier} ($n{=}10$ each). Skip rate scales monotonically ($24\%$ / $34\%$ / $51\%$). The $\sim$3.7~pp anytime gain at 15--20k evaluations is therefore not an artifact of a single $\tau$: at $\tau{=}0.45$ filter leads 10/10 seeds at both budgets; at $\tau{=}0.35$ mid-budget gain is smaller ($+2.5$~pp @15k, 9/10) but still positive on most seeds; at $\tau{=}0.55$ it is larger at 15k ($+5.4$~pp, 10/10) but terminal coverage falls further ($56.4\%$, paired $\Delta{=}{-}2.6$~pp vs.\ uniform). Aggressive gates trade terminal coverage for faster mid-budget fill; the primary-grid $\tau{=}0.45$ sits in the middle of this trade-off.
...
After H1 and H2, H3 asks whether adding the same after-generation gate to the LLM$+$hint stack helps further. Confirmatory non-inferiority on the \textbf{locked production filter} remains blocked ($d{=}0.528 > 0.05$ at $\tau{=}0.45$); we report those levels as exploratory and do \emph{not} run new confirmatory seeds here.

\textbf{Why confirmatory H3 is blocked (summary).} Hard fitness recompose zeros a candidate when predicted $p_{\mathrm{ext}}\ge$ an extinction gate. Historical combat logs used live gate~$0.95$ while the locked skip-agreement check compared gates $0.5$ vs.\ $0.95$; most proposals sit in the gray band $p_{\mathrm{ext}}\in[0.5,0.95)$, so the two definitions disagree on skips. Offline replay localizes the block to that dual compose definition---not to an opaque pipeline failure or a single mis-set $\tau$. Hold-out validity is now aligned to gate~$0.95$ (\S\ref{sec:checkpoint}); that does \emph{not} retroactively pass the locked skip-agreement metric. Full $\tau\times$emitter replay, fix-candidate skips, and the deferred gray-zone repair plan are in appendix~\S\ref{sec:h3-appendix}.

On the bundled LLM tier, locked production \texttt{filter} vs.\ \texttt{hints} is $-1.14$~pp coverage at fixed iterations ($59.27 \pm 0.79\%$ vs.\ $60.41 \pm 0.82\%$). Stacking both slots is not a simple sum of H1$+$H2. Primary after-generation evidence stays H2 (matched genetic arms; \S\ref{sec:h2}). Dungeon/maze show the same partial pattern (H2 ports; LLM$+$filter does not; \S\ref{sec:h5}).

\subsection{H4 --- Continuous-relaxation pyribs reference (CMA-ME / CMA-MAE)}
\label{sec:h4}

Pyribs baselines (five homogeneous CMA-ES emitters, $n=10$) provide a \textbf{protocol control} under the same warm-start and evaluation budget as other primary-grid arms. Because CMA searches a continuous relaxation with \texttt{rint} decode (\S\ref{sec:pyribs}), this section reports an \textbf{upper-bound-style reference for continuous-friendly representations}---not a validated discrete coverage ceiling against genetic/LLM bit-flip emitters. Mean levels: cma\_me $73.37 \pm 3.65$\%, fitness $0.576 \pm 0.028$; cma\_mae $60.56 \pm 2.16$\%, fitness $0.489 \pm 0.007$; hints $60.41 \pm 0.82$\%, fitness $0.539 \pm 0.008$.
...
\begin{table}[!htbp]
  \centering
  \caption{Pyribs continuous-relaxation reference (H4): Holm step-down ($m=4$, paired hints $-$ arm, one-sided greater). Not an H2 contrast. $\mathrm{PPS}$ (Eq.~\eqref{eq:pps}) is descriptive only---not $A_{12}$, not a propensity score, and not a confirmatory gate.}
...
We replicated the bundled LLM+constants vs.\ LLM+scalars contrast on two additional providers with identical schedulers (6{,}500 LLM calls per run). Table~\ref{tab:g1} reports each provider separately---never pooled. Both additional providers pass the per-provider bundled contrast at $n{=}10$ (Holm $m{=}2$ on conjunctive $\Delta$cov+$\Delta$fit; both reject per provider). \textbf{This is not matched H1:} the multi-provider table keeps the historical stub arm (\texttt{min\_fitness\_frontier}), so provider replication supports the confounded stack $\Delta$, not the matched-policy \texttt{stub\_uniform}-vs-\texttt{hints} null.

\begin{table}[!htbp]
  \centering
  \caption{Multi-provider: \emph{bundled} hints $-$ stub \emph{by} LLM provider (confounded with \texttt{target\_selection}). Not matched-policy H1. Absolute hint-arm coverage shows level gaps across APIs. \textbf{Not to be read as a combined effect:} rows are separate experiments (never pool or average $\Delta$ across LLMs); Holm applies per provider at matched $n$.}
  \label{tab:g1}
  \fitcenter{%
...
\textbf{Scope of this subsection.} We ask whether the after-generation surrogate \emph{ports} beyond the primary evaluator, not whether the bundled LLM+surrogate stack generalizes. The locked confirmatory check is a 16$\times$16 patch-LLM dungeon under frozen v4 ($n{=}10$, 5{,}000 proposals/arm). A journal-track maze factorial at the primary-grid proposal budget (32{,}500 proposals/arm) is reported as an interim third-domain slice below. Neither is a claim of cross-domain LLM QD superiority. Timing-to-coverage thresholds from the first dungeon pilot remain exploratory; confirmatory endpoints are AUC at matched real evaluations. Dungeon quality gates passed (LLM fallback $\le 0.99\%$, filter skip 34--40\%). Figure~\ref{fig:b4} shows dungeon anytime curves. A supplementary dungeon CPU after-generation pair at 32{,}500 proposals is reported in Table~\ref{tab:h2-cpu-full}; full-budget LLM dungeon arms remain deferred.

\textbf{Filter thresholds (operating points, not swept).} Each domain uses its own fitness-gate $\tau$, chosen by offline shadow replay to land in a locked skip band ($\sim$25--45\%): primary grid $\tau{=}0.45$ (production skip $33.5\%$), dungeon $\tau{=}0.78$ (skip $34$--$40\%$ @ 5k; $30.8\%$ @ 32.5k CPU), maze $\tau{=}0.685$ (skip $\sim$27\%). Absolute $\tau$ values are not transferable across surrogates/fitness scales; the shared design rule is the skip band. Primary-grid H2 has an explicit online $\tau$ sweep at $\{0.35,0.45,0.55\}$ (Table~\ref{tab:h2-tau-sweep}): mid-budget filter advantage stays directionally positive while magnitudes trade off against terminal coverage. \emph{No} matched online $\tau$ sweep is reported for dungeon or maze in this version, so the H5 ``H2 ports'' readout is at those calibrated operating points---not shown to be $\tau$-invariant within each secondary domain. A dungeon $\tau$ ablation is a deferred journal-extension item (protocol v4).
...
\begin{table}[!htbp]
  \centering
  \caption{Maze Holm outcomes (interim H5; $m{=}8$; AUC @ 22{,}860 real evaluations, seeds $0$--$4$). No contrast survives Holm at $n{=}5$ (underpowered; retrospective MDE ${\approx}4.7$~pp). Sign concordance only---not a confirmatory dungeon replica. Replace with $n{=}10$ when seeds 5--9 LLM arms complete.}
  \label{tab:b5-holm}
  \small
  \fitcenter{%
  \begin{tabular}{@{}lrrc@{}}
...
\textbf{H5 readout (dungeon + interim maze).} After-generation sample-efficiency without LLM transfers on both secondary domains \emph{at their calibrated skip-band $\tau$} where $n{=}10$ Holm is available (dungeon H2 AUC; maze CPU H2 AUC). The maze five-arm interim ($n{=}5$) is sign-consistent with that H2 direction but underpowered for Holm $m{=}8$ and is \emph{not} a confirmatory H5 replica. Bundled LLM QD lift does \emph{not} transfer on dungeon ($n{=}10$): genetic MAP-Elites leads LLM arms by $\sim$19~pp on coverage AUC at matched evals; maze shows a similar descriptive gap ($\sim$40~pp on the interim slice). Dungeon hints$-$stub is a small AUC edge ($+0.85$~pp; retrospective MDE ${\approx}1.2$~pp, Eq.~\eqref{eq:mde}) that does not survive Holm---a fail-to-reject, not TOST equivalence. LLM$+$filter interaction stays negative (wrong-direction / near-zero; not an underpowered ``true zero'' claim). Robustness of those transfer rejects/accepts to alternate $\tau$ within each secondary domain is not tested here (primary-grid sweep only; Table~\ref{tab:h2-tau-sweep}).
...
Taken together, the results indicate that, for LLM-guided QD, \emph{where} a surrogate attaches can matter more than which pipeline wins a bake-off (Figure~\ref{fig:pipeline}). Under one shared MLP checkpoint on this evaluator, the after-generation surrogate (H2; Figure~\ref{fig:acquisition}) improves coverage \emph{per simulation} (eval-count axis only---not terminal coverage or wall clock here), while the matched-policy before-generation surrogate (H1) stays within $\pm 2$~pp at $n{=}10$ on \texttt{qwen-turbo} (post-hoc TOST)---not yet a full matched-power replication on other LLMs. That asymmetry is the intended payoff of attachment-point decomposition: the after-generation surrogate hard-gates evaluation spend (SAIL-\emph{kin}), whereas the before-generation surrogate supplies prompt-side advice that never skips \texttt{evaluate\_candidate}. H3 levels further imply that stacking both under LLM is not a simple sum; confirmatory interaction testing is deferred (dual compose gate diagnosed; gray-zone repair; \S\ref{sec:h3}, \S\ref{sec:limitations}). Magnitudes are testbed-specific; the before/after framing is intended as a reusable question for LLM-guided QD stacks, not a universal ranking.
...
\begin{itemize}
 ...
 Skip logs and matched contrasts are consistent with the after-generation surrogate mainly reallocating simulator spend rather than inventing stronger proposals. Skip logs (\S\ref{sec:why-filter}) show discarded proposals concentrated at low predicted fitness (mean $0.21$ vs.\ $0.61$ evaluated), high extinction risk ($81\%$ of skips with $p>0.8$), and low diversity, while uncertainty barely separates skip from evaluate. At matched evaluations that discarded mass frees slots for niches that can still fill ($+3.65$~pp @ 20k); at fixed iterations the same gate finishes $-0.83$~pp---a pattern consistent with reallocation under a fixed proposal schedule.
...
\textbf{Honest efficiency readout first.} If the decision criterion is archive coverage, wall time, or API dollars on this evaluator, the answer to ``should I use an LLM emitter?'' is mostly \emph{no}: continuous-relaxation CMA-ME leads the LLM+\texttt{hints} stack by $\sim$13~pp coverage (H4); even native bit-flip CMA-ME leads by $\sim$5~pp; matched-policy genetic ME is within $\approx{+}1.4$~pp of hints at roughly $10\times$ lower wall and \$0 API (\S\ref{sec:cost}, Figure~\ref{fig:ladder}); dungeon and maze reverse the ranking further toward genetics. Those facts are primary for a metrics-first reader and are not buried behind the attachment-point story.

The scientific object of this paper is nevertheless surrogate \emph{placement} inside LLM-guided QD loops that \emph{already} use an LLM as a variation operator (Figure~\ref{fig:pipeline})---conditional on that design choice, not as a bake-off win for LLMs. H2's primary arm drops the LLM (\texttt{genetic\_me\_filter}) so the after-generation attachment can be isolated on the evaluation axis. When the objective is sample efficiency among \emph{discrete} emitters at matched policy, genetic$+$after-generation (H2) is the attach-role winner on that axis alone (\S\ref{sec:h2})---not a terminal-coverage or wall-clock recommendation; absolute rankings that invoke CMA-ME inherit the continuous-relaxation caveat (\S\ref{sec:h4}).

A remaining qualitative niche for LLM emitters is \emph{structured, human-readable variation} (named-field edits vs.\ anonymous continuous coordinates; appendix~\S\ref{sec:substrate-appendix}). We do \emph{not} quantify the value of interpretability (no user study or editable-design case study). Dungeon/maze reverse the LLM vs.\ genetic coverage ranking, so semantic edits need not improve QD metrics once policy is matched. Practical reading: use an LLM only when editable structured genotypes are themselves a product requirement; otherwise prefer genetic$+$after-generation (or a CMA-style baseline) for archive metrics, and attach any shared surrogate primarily after generation if an LLM is retained.
...
The fixed-slot decomposition leaves room for \textbf{adaptive integration} (and, separately, adaptive emitter scheduling):
...
\textbf{Threats to validity.} Primary evidence comes from a cheap evaluator where wall time is dominated by MLP inference, so H2's evaluation-indexed gain need not translate into a wall-clock win here. The pyribs ``coverage'' contrast uses continuous-relaxation encoding; decode ablations do not remove that confound because they still search $\mathbb{R}^{21}$ before rounding. H3 is exploratory: the locked production filter fails compose-gate skip agreement, but offline replay localizes the mechanism (gray-zone dual-definition; in-band vs.\ out-of-band $d$) and documents corrected hold-out validity (NMAE degradation); a confirmatory gray-zone repair is deferred. LLM endpoints are window-pinned but floating aliases limit bit-exact replay. Holm correction is within locked contrast sets; paper-wide Holm agrees on claim-driving rows (Table~\ref{tab:paper-holm}). Hypotheses and analysis rules are internally frozen in dated repository protocol files with a reader amendment summary (Table~\ref{tab:amendments}) rather than a public OSF/Zenodo pre-registration.

\textbf{Testbed scope.} Primary contrast sets use the grid evaluator in \S\ref{sec:substrate}; a dungeon factorial is a second LLM-guided QD domain, and an interim maze factorial is a third. All three are procedurally generated discrete environments with cheap simulators relative to CFD-style SAIL domains. Magnitudes (e.g.\ $+3.7$~pp, $33.5\%$ skips) are therefore testbed-specific; the before/after split is offered as a reusable \emph{question} for LLM-guided QD stacks, not as a universal ranking that has been shown to transfer to expensive continuous genomes or design/CFD evaluators. H2's positive result is scoped to the evaluation-count axis on this testbed (three practical axes: eval-count positive; terminal coverage and wall-clock negative). Evaluation-gating is conceptually SAIL-\emph{kin} and could in principle help when $t_{\mathrm{sim}}$ dominates wall time (break-even $\sim$7$\times$ slower simulators on this stack; \S\ref{sec:cost}), but that wall-clock transfer is an untested extrapolation---we neither slow the primary simulator artificially nor run a CFD-cost domain in this version. Prompt-side attachment (H1) is most natural for structured, human-readable genotypes; a pure $\mathbb{R}^d$ continuous-control domain would stress a different emitter/prompt interface and is out of scope for this version. Dungeon and maze transfer filter AUC but not bundled LLM QD lift; maze genetic arms additionally hit a 900-cell ceiling that makes terminal coverage uninformative. Sphere/Rastrigin pyribs tiers in the repository are engineering smoke checks of our glue code (not scientific claims and not a validation of upstream pyribs).

\textbf{Filter-threshold calibration (H5).} Primary-grid H2 reports an online $\tau$ sweep (Table~\ref{tab:h2-tau-sweep}). Dungeon ($\tau{=}0.78$) and maze ($\tau{=}0.685$) gates were set by shadow replay to a shared skip-band target ($\sim$25--45\%), not by repeating that online sweep. The H5 ``after-generation ports'' claim is therefore conditional on those operating points; we do not claim $\tau$-invariance of transfer within dungeon or maze. A dungeon $\tau$ ablation remains a journal-extension item.
...
\textbf{Protocol freezing.} Hypotheses and analysis rules are locked in dated repository protocol files (primary grid: 2026-07-12; dungeon: 2026-07-17). Post-freeze amendments are summarized in Table~\ref{tab:amendments} (full dual-report logs in-repo). The maze factorial is a journal-track H5 draft; this draft reports an interim $n{=}5$ five-arm slice and a complete $n{=}10$ CPU H2 pair. This submission uses \emph{internally frozen} wording only---no OSF/Zenodo deposit (optional for a later revision).

\textbf{Manuscript scope.} This draft reports all frozen primary-grid confirmatory contrast sets (v2/v3) and locked dungeon H5 on the \emph{5k five-arm matrix} ($n{=}10$; partial). Supplementary dungeon evidence adds the CPU H2 pair at the primary-grid proposal budget (Table~\ref{tab:h2-cpu-full}). An interim maze five-arm slice (seeds $0$--$4$) and maze CPU H2 pair ($n{=}10$; Tables~\ref{tab:b5},~\ref{tab:h2-cpu-full},~\ref{tab:b5-holm}) are included; the full maze $n{=}10$ five-arm Holm replaces the interim slice when seeds 5--9 complete. Full-budget LLM dungeon arms, a \texttt{random} MAP-Elites baseline @ 32.5k, and one dungeon ablation remain deferred. Bernoulli-decode, native bit-flip, and pbCMA supplementary arms ($n{=}10$ each) are reported in \S\ref{sec:h4}.

\textbf{Power on non-reject / exploratory contrasts.} Only matched-policy H1 carries a formal TOST/MDE (Table~\ref{tab:decomposition}). For other non-rejects we do not claim ``true zeros'': dungeon H5 hints$-$stub and LLM$+$filter Holm failures are fail-to-reject (retrospective MDE ${\approx}1.2$~pp on hints$-$stub coverage AUC at $n{=}10$; observed mean $+0.85$~pp); H3 is compose-gate-blocked rather than an underpowered interaction test; H4's CMA-ME miss is a large reverse effect. Maze five-arm Holm at $n{=}5$ is underpowered (retrospective MDE ${\approx}4.7$~pp on genetic\_filter coverage AUC---matching the observed mean before Holm $m{=}8$) and is labeled interim; confirmatory maze H2 rests on the $n{=}10$ CPU pair.
...
\textbf{Acquisition ablation scope.} The GP+UCB comparison is offline (hold-out regression + acquisition replay) plus a counterfactual on logged MLP predictions; it does not re-run the confirmatory filter matrix under GP or UCB. Hold-out MLP vs.\ GP regression was recomputed at runtime-aligned compose gate~$0.95$ (MLP $R^2{=}0.94$ / NMAE$=0.112$ vs.\ GP $0.89$ / $0.222$); the legacy gate-$0.5$ snapshot ($0.76$ vs.\ ${\approx}0.22$) is retained only as a cautionary dual-report. Compose-gate vs.\ $\tau$ replay on production \texttt{filter} proposals (\S\ref{sec:h3}) is likewise offline on logged proposals.
...
\textbf{Pipelines and instrumentation.} The CMA ceiling contrast compares complete QD pipelines (homogeneous five-emitter CMA pools vs.\ heterogeneous MAP-Elites+LLM), not a factorial decomposition of emitters. A post-hoc decode/cold-start ablation (seeds~0--4), Bernoulli-decode CMA-ME ($n{=}10$), native bit-flip CMA-ME ($n{=}10$), and pbCMA CMA-ME ($n{=}10$; \S\ref{sec:h4}) show the hints$\ll$CMA gap survives alternate decodes ($+10.8$~pp Bernoulli vs.\ hints; $+9.5$~pp pbCMA; $+5.2$~pp native vs.\ hints) but only pbCMA/Bernoulli approach \texttt{rint} on mean coverage ($-3.4$ / $-2.1$~pp). Matched evaluation budget is not matched dollar cost (\S\ref{sec:cost}).
...
Having studied \emph{fixed} attach roles (Table~\ref{tab:role-exp}), a natural next step is \textbf{adaptive integration}: when to activate the before- vs.\ after-generation surrogate, and when to call LLM vs.\ genetic emitters (\S\ref{sec:bootstrap-hyp}). \textbf{Journal extension candidates:} (i)~align hold-out compose gate to production ($0.95$; done in code for this revision) and run a confirmatory gray-zone H3 repair (\texttt{filter\_gray\_zone} vs.\ \texttt{hints}, $n{=}10$): gray-zone \texttt{force\_eval} policy is the offline fix candidate ($d{\rightarrow}0$); optional $n{=}3$ exploratory pilot first (Table~\ref{tab:hint-pilots} precedent); (ii)~complete maze five-arm Holm at $n{=}10$ (seeds 5--9 LLM arms) to replace the interim H5 slice; (iii)~full-budget LLM dungeon factorial, \texttt{random} baseline @ 32.5k, and dungeon/maze filter-$\tau$ ablations under a dated protocol amendment (primary-grid $\tau$ sweep already in Table~\ref{tab:h2-tau-sweep}); (iv)~matched-policy H1 at $n{=}10$ on additional LLM providers (gpt-4o-mini pilot $n{=}3$ is exploratory only); (v)~an artificial sim-sleep / cost-scaled (or continuous-genome) domain to \emph{measure} whether evaluation-gating becomes a wall-clock win near the $\sim$7$\times$ break-even forecast, and whether the before/after asymmetry survives outside cheap discrete PCG stacks. An external OSF/Zenodo protocol timestamp remains optional for a later revision.

\section{Conclusion}
\label{sec:conclusion}

Previous work often compared QD pipelines; we compared attachment points. The EC object is surrogate model management in MAP-Elites: information can enter the propose$\rightarrow$evaluate loop before or after generation (Figure~\ref{fig:pipeline}), with an LLM only as one variation operator. Under one shared checkpoint on our controlled evaluator, only the after-generation attach role (H2) improves the evaluation-count axis (Figure~\ref{fig:acquisition}): $\sim$3.7~pp mid-budget coverage per simulation and 33.5\% fewer sims at fixed iterations---not terminal coverage ($-0.83$~pp) or wall clock ($\sim$20 vs.\ $\sim$7~min); expensive-domain wall transfer remains extrapolated. Matched-policy H1 on \texttt{qwen-turbo} (\texttt{stub\_uniform}, $n{=}10$) is \emph{descriptively flat} within $\pm 2$~pp (post-hoc TOST; MDE$\approx$1.2~pp)---exploratory, \emph{not} a confirmatory null; other providers replicate the bundled stack only. H3 levels are exploratory: stacking both under LLM is not a simple sum; we diagnose the blocked confirmatory path (dual compose gate; appendix~\S\ref{sec:h3-appendix}) and defer the gray-zone repair.

Genetic ME already sits within about $1.4$~pp of the full LLM+scalar stack; continuous-relaxation CMA-ME leads hints by $\sim$13~pp as an encoding-asymmetric upper-bound reference (Bernoulli $\sim$11~pp; native bit-flip $\sim$5~pp; \S\ref{sec:h4})---none is a validated discrete winner over genetic$+$after-generation at matched policy. On a second domain, dungeon H5 is \textbf{partial} (Table~\ref{tab:families}): genetic\_filter$-$genetic AUC passes Holm, but hints$-$stub and LLM$+$filter interaction do not---after-generation engineering ports; bundled LLM QD lift does not. We release an internally frozen stack (public GitHub repository; submission pin tag~\texttt{\SubmissionGitTag} / commit~\texttt{\SubmissionGitShort} in Data availability) so others can re-run the attachment-point split on their own LLM-guided QD loops. \textbf{Caveats:} see \S\ref{sec:limitations}.
...
Matched-policy H1 is exploratory (not a confirmatory Holm set). We use a post-hoc two one-sided tests (TOST) equivalence check at $|\Delta\mathrm{cov}|\le 2$~pp. Approximate minimum detectable effect for a paired $t$-test at $n$ seeds, two-sided $\alpha{=}0.05$, 80\% power is
\begin{equation}
...
\paragraph{Fix candidates and deferred confirmatory path.}
Offline fix-candidate replay (\texttt{compose\_gate\_fix\_candidates.json}) confirms out-of-band proposals have $d{=}0$; force-eval of the gray zone (or skip only when $p_{\mathrm{ext}}\ge 0.95$) drives $d\!\rightarrow\!0$ but cuts skip from $33.5\%$ to $11.7\%$. Soft extinction at $\tau{=}0.45$ over-skips ($96\%$); retargeting soft $\tau$ to $\approx 0.11$ recovers $\sim$32\% skip with $0.038$ skip-divergence vs.\ hard@$0.95$. Aligning both compose gates to live $0.95$ zeroes $d$ by construction for \textbf{new} runs---not for the locked historical replay metric. Protocol 2026-07-28 therefore pre-specifies a deferred arm \texttt{filter\_gray\_zone} (\texttt{force\_eval\_extinction\_gray\_zone} when $p_{\mathrm{ext}}\in[0.5,0.95)$) vs.\ matched \texttt{hints} ($n{=}10$; locked non-inferiority margins; YAML \texttt{map\_elites\_scheduler\_nightly\_llm\_filter\_gray\_zone.yaml}; shadow skip target $8$--$18\%$). Optional $n{=}3$ pilot first (Table~\ref{tab:hint-pilots} precedent). Passing that repair would not retroactively validate historical production \texttt{filter} at 33.5\% skip.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}llp{3.4cm}cl@{}}
    ...
    \midrule
    H1 (before-gen) & \texttt{stub\_uniform} vs.\ \texttt{hints} & --- & exploratory; TOST$\pm$2~pp accept \\
    Shared-checkpoint gate & Hold-out $R^2$/MAE & --- & PASS \\
    H2 (after-gen) & \texttt{genetic\_me\_uniform} vs.\ \texttt{genetic\_me\_filter} & --- & eval-count gain (not terminal/wall) \\
    H3 (interaction) & \texttt{hints} vs.\ \texttt{filter} & --- & exploratory; compose-gate blocked \\

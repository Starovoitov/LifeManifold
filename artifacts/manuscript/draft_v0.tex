...
On the nightly buffer hold-out (compose gate~0.95, aligned with runtime), the production MLP achieves $R^2{=}0.94$ (95\% CI $[0.94, 0.95]$) vs.\ ${\approx}0.22$ for a scikit-learn GP fitness regressor trained on 5{,}000 rows at the same gate, and is ${\sim}12\times$ faster at a 32{,}500-proposal batch.
...
The locked compose-gate check for LLM+filter non-inferiority remains \textbf{blocked}: live proposal replay at threshold~0.45 yields divergent skip rate $d=0.528$ (required $\le 0.05$). Offline replay stratified by emitter at $\tau\in\{0.35,0.45,0.55\}$ shows $d$ is $\tau$-dependent but stays $\gg 0.05$ in the operating band (\S\ref{sec:h3}; Table~\ref{tab:h3-compose-tau}). H3 is therefore \textbf{descriptive / exploratory by design} in this version---not a deferred footnote to a confirmatory claim. H2 reports matched \texttt{genetic\_me\_uniform} vs.\ \texttt{genetic\_me\_filter} as the primary filter evidence (\S\ref{sec:h2}).
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}llp{6.4cm}@{}}
    ...
    Hold-out (shared-checkpoint validity) & $R^2{=}0.94$ $[0.94,0.95]$, MAE$_{\text{fit}}{=}0.029$ $[0.027,0.030]$ & $n{=}7{,}023$ hold-out; compose gate $0.95$ aligned with runtime; bootstrap 95\% CI; \texttt{hints\_ok} PASS \\
...
Offline training on the nightly buffer ($N=35{,}114$ rows; schema~2.1, 24-D features; hold-out $n{=}7{,}023$) yields hold-out metrics for checkpoint \texttt{nightly\_v3\_mc\_d005.pkl} with **aligned** extinction compose gate $0.95$ (protocol amendment 2026-07-28; matches runtime YAML): $R^2_{\text{fitness}}=0.94$ (bootstrap 95\% CI $[0.94, 0.95]$), MAE$_{\text{fitness}}=0.029$ ($[0.027, 0.030]$), MAE$_{\text{stability}}=0.040$ ($[0.038, 0.042]$), passing both the \texttt{hints\_ok} and production quality gates. CIs resample hold-out rows ($B{=}10{,}000$). Prior headline metrics at misaligned gate~0.5 were $R^2{=}0.76$ [$0.70{,}0.82$], MAE$_{\text{fit}}{=}0.007$---artifact \texttt{holdout\_compose\_gate\_alignment.json}. Shared-checkpoint validity is therefore supported at the offline gate aligned with live compose. Hold-out metrics do not replace simulator endpoints for H1/H2/H3.
...
After H1 (before-generation) and H2 (after-generation), H3 asks whether adding the same after-generation gate to the LLM$+$hint stack helps further. Confirmatory non-inferiority remains blocked at the locked production threshold ($d{=}0.528 > 0.05$ required at $\tau{=}0.45$), so we report levels only. On the bundled LLM tier, \texttt{filter} vs.\ \texttt{hints} is $-1.14$~pp coverage at fixed iterations ($59.27 \pm 0.79\%$ vs.\ $60.41 \pm 0.82\%$). Stacking both slots is not a simple sum of H1$+$H2. Primary after-generation evidence stays H2 (matched genetic arms; \S\ref{sec:h2}). Dungeon and interim maze factorials give the same partial transfer pattern for filter AUC (H2 ports; LLM$+$filter interaction does not; \S\ref{sec:h5}).

...
Reader-facing contrast labels are \textbf{H1}--\textbf{H5} (Table~\ref{tab:role-exp}). Prompt placeholders and threshold gates are mechanisms \emph{inside} a role, not alternate role names. Historical protocol family IDs (\texttt{RQ*}, \texttt{F-RQ*}, \texttt{F-B4-*}) appear only in the appendix crosswalk (Table~\ref{tab:families}).
...
\textbf{Threshold sensitivity (pilot).} A matched-policy sweep (\texttt{q1-h2-threshold-sensitivity}; Table~\ref{tab:h2-tau-sweep}) varies $\tau\in\{0.35,0.45,0.55\}$ at fixed \texttt{uniform\_frontier}. Skip rate scales monotonically ($24\%$ / $34\%$ / $50\%$). The $\sim$3.7~pp anytime gain at 15--20k evaluations is therefore not an artifact of a single $\tau$: at $\tau{=}0.45$ ($n{=}10$) filter leads 10/10 seeds at both budgets; at $\tau{=}0.35$ ($n{=}3$) mid-budget gain is smaller ($+2.7$~pp @15k, 2/3 seeds); at $\tau{=}0.55$ ($n{=}3$) it is larger at 15k ($+6.2$~pp, 3/3) but terminal coverage falls further ($57.1\%$, paired $\Delta{=}{-}2.3$~pp vs.\ uniform). Aggressive gates trade terminal coverage for faster mid-budget fill; the primary-grid $\tau{=}0.45$ sits in the middle of this trade-off.
...
\begin{table}[!htbp]
  \centering
  \caption{Threshold sensitivity for \texttt{genetic\_me\_filter} vs.\ matched \texttt{genetic\_me\_uniform} (descriptive pilot). Anytime $\Delta$ is filter $-$ uniform coverage at matched evaluation budget; terminal $\Delta$ at fixed 650 iterations.}
  ...
  \begin{tabular}{@{}lrrrrrr@{}}
    ...
    \midrule
    0.35 & 3 & $24.1\%$ & $58.20\%$ & $+2.70$ (2/3) & $+2.43$ (2/3) & $-1.17$ \\
    0.45 & 10 & $33.5\%$ & $58.21 \pm 1.03\%$ & $+3.72$ (10/10) & $+3.67$ (10/10) & $-0.83$ \\
    0.55 & 3 & $50.4\%$ & $57.07\%$ & $+6.15$ (3/3) & $+3.59$ (3/3) & $-2.31$ \\
    \bottomrule
  \end{tabular}%
  }
\end{table}
...
\begin{itemize}
 ...
 Primary H2 uses $\tau{=}0.45$; a pilot sweep at $\tau\in\{0.35,0.45,0.55\}$ (\S\ref{sec:why-filter}; Table~\ref{tab:h2-tau-sweep}) shows mid-budget gains scale with skip rate while aggressive $\tau$ hurts terminal coverage. Full $n{=}10$ at all $\tau$ values remains optional.
...
These observations point to explanatory gaps that still matter more than new LLMs or domains (\S\ref{sec:why-discussion}): (1)~\textbf{phase analysis}---extend anytime curves to genetic ME and mid-trajectory H2 vs.\ hints; (2)~\textbf{prompt micro-ablation} (drop uncertainty / predicted fitness / archive stats) to refine the soft-channel reading of H1. A pilot H2 threshold sweep at $\tau\in\{0.35,0.45,0.55\}$ is reported in Table~\ref{tab:h2-tau-sweep}; extending to $n{=}10$ at $\tau{=}0.35/0.55$ or a wider $\tau$ grid is optional.

Having studied \emph{fixed} attach roles (Table~\ref{tab:role-exp}), a natural next step is \textbf{adaptive integration}: when to activate the before- vs.\ after-generation surrogate, and when to call LLM vs.\ genetic emitters (\S\ref{sec:bootstrap-hyp}). \textbf{Journal extension candidates:} (i)~compose-gate repair followed by confirmatory H3 if the gate passes; (ii)~full-budget LLM dungeon factorial, \texttt{random} baseline @ 32.5k, and one dungeon ablation family under a dated protocol amendment; (iii)~matched-policy H1 at $n{=}10$ on additional LLM providers (gpt-4o-mini pilot $n{=}3$ is exploratory only). An external OSF/Zenodo protocol timestamp remains optional for a later revision.

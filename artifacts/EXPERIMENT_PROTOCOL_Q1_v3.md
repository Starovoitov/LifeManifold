...
**External timestamp:** Zenodo integrity snapshot **10.5281/zenodo.22119553** (published **2026-08-27**; git tag `journal-v1.1` / `a714e548cfd6`) — current artifact pin of concept **10.5281/zenodo.21727010**. Version 1 **10.5281/zenodo.21727011** (`journal-v1` / `6d07dcdecf64`, **2026-07-31**) remains the first deposit of that concept. *Not* a prospective pre-registration of completed runs; protocols remain frozen per family in this file + v2/v4/v5.  
...
| Seeds | **0–9** |
| Warm-start | **Yes** — same grid baseline as RQ0/v2 ME for every seed/arm: `artifacts/map_elites_nightly/baseline/map_elites_archive.jsonl`. **Counts:** 2 614 JSONL records → **971** occupied cells after `load_and_collapse_jsonl` (strict fitness `>`; first wins ties) → **38.84%** initial coverage, QD-score \(\sum f_i\approx417.9\) (mean elite fit ≈0.430). pyribs `warm_start_elites` logs raw line count (2 614), not collapsed niches. |
...
**Manuscript reporting (2026-08-07; category correction, not demotion):** main-text H4 is a **descriptive continuous-relaxation ceiling**, because confirmatory “hints > CMA” was a **category error** given the encoding asymmetry already disclosed at freeze (CMA on $\mathbb{R}^{21}$ + `rint` vs discrete LLM/genetic edits). **Dual-report:** locked F-RQ4 (Holm **m=4**) remains **FAIL** on unchanged numbers; that FAIL is retained in the audit trail alongside the descriptive ceiling reading. Multiplicity stays **within-family only** (no pooled paper-wide Holm). See §12 rows 2026-08-07 / 2026-08-09.
...
| **Pairs** | `hints` vs `cma_me` seeds **4 / 6 / 1** (largest / mid / smallest \|Δcov\| = most / mid / least negative); optional `filter` vs `cma_me` seed **4** |
| **Deliverable** | Per-seed PDFs + `archive_heatmaps/*.png` + panel `fig07_archive_heatmaps_panel_seeds4_6_1.pdf`; main text Fig.~7 = seed 4. **n=10 occupancy:** `fig07_occupancy_n10.pdf` — $P(\mathrm{occ})$, $\mathbb{E}[f\mid\mathrm{occ}]$, $\Delta P$ (cma_me−hints). Axes $[0,1]^2$ (Diversity →, Stability →); gray = empty, not infeasible. |
| **Illustrative vs aggregate** | Seeds 4/6/1 remain outcome-selected illustrations; occupancy/ΔP is the n=10 spatial companion (not a confirmatory spatial test). |
| **Reproduce** | `.venv/bin/python scripts/export_manuscript_figures.py --fig 7 --panel` (default seed order 4,6,1; also writes occupancy) |
...
- [x] **Archive heatmap figures (§3.11):** seeds 4/6/1 hints vs CMA-ME + panel; n=10 occupancy/ΔP (`fig07_occupancy_n10`); optional filter seed 4; under `artifacts/manuscript/figures/`
...
| 2026-08-07 | **H4 / F-RQ4 manuscript category correction (Reporting):** **was:** H4 listed as confirmatory Holm family **m=4** (F-RQ4). **now:** main-text H4 reported as a **descriptive continuous-relaxation ceiling** because confirmatory “hints > both CMA arms” is a **methodological category error** under the encoding asymmetry disclosed at the 2026-07-12 freeze (CMA continuous $\mathbb{R}^{21}$ + `rint` vs discrete LLM/genetic genomes)—not a result-driven demotion after FAIL. **Dual-report (numbers unchanged):** locked F-RQ4 remains **FAIL** (only `RQ4_mae_fit` Holm-rejects; CMA-ME Δcov mean **−12.96** pp). Frozen hypothesis text in §3.2 / §4.1 unchanged; artifact `q1-v3-pyribs/frq4_statistics.json` unchanged. |
| 2026-08-09 | **Withdraw pooled “paper-wide” Holm (Reporting):** manuscript drops the incomplete $m{=}16$ pooled Holm table (bundled+floor+dungeon+multi-provider only; missing maze H5 / H3-gray / F-RQ4). Multiplicity remains **within-family only**. Does **not** amend any locked family outcomes. |
| 2026-08-09 | **H2 break-even wording (Reporting):** §h2-breakeven clarifies accounting identity under fixed proposals; unconditional $t_{\mathrm{sim}}$ can overstate savings when skips are early-extinction; ~7× is implementation-specific; LLM after-generation gate does not cut API; injected-delay sleep is only inside `evaluate_maze` on simulated proposals (`q1-v5-maze-cost-h2`). Numbers unchanged. |
| 2026-08-09 | **H2 online gate validation + 24-D feature list (Reporting):** shadow-stream AUROC/precision-recall/Spearman/emitter NMAE at $\tau{=}0.45$ (`scripts/analyze_h2_gate_online_validation.py`); manuscript Eq. features-24 enumerates 21 genome params → 24-D schema 2.1 (+ birth/survival/overlap cardinalities). Does **not** amend Holm families. |
| 2026-08-09 | **Paired seeds vs LLM stochasticity + simulator seed rule (Reporting):** manuscript clarifies run-seed pairing does not control external LLM sampling ($T{=}0.2$, no provider seed, non-interleaved frozen arms, no call logs); simulator fitness/BC = one pass under genotype-canonical SHA-256 seed (re-sim panels match archive labels). Numbers unchanged. |
| 2026-08-09 | **H1 policy×hint missing cell DONE (descriptive):** tier `q1-h1-policy-x-hint` / `hints_minfit` $n{=}10$. Soft@minfit **−0.02±0.05** pp (8/10 identical to `stub`); soft@uniform **+0.26**; policy@stub **+14.83** / policy@hints **+15.12**; interaction **+0.28** (n.s.). Protocol [`Q1_H1_POLICY_X_HINT.md`](Q1_H1_POLICY_X_HINT.md); manuscript Table `tab:policy-x-hint`. Does **not** amend confirmatory H1/Holm. |
| 2026-08-09 | **H2 inferential-status clarification (Reporting):** manuscript now marks matched genetic H2 (`genetic_me_uniform` vs `genetic_me_filter`; cov/AUC@20k) as **descriptive** per §3.8 (no Holm family); Table `tab:h-status` adds Status column (C/E/D/Dual). Does **not** amend §3.8 lock or invent F-RQ2. |
| 2026-08-09 | **Proposal vs simulator budget terminology (Reporting):** manuscript now keeps three budgets distinct—(i) matched **proposal slots** (32{,}500), (ii) **real simulator evaluations** (filter arms ${\sim}21.5$k), (iii) wall/API cost. Wording that equated 32{,}500 slots with sims for filter arms is corrected. Does **not** change locked endpoints, skip rates, or numeric results. |
| 2026-08-09 | **H5/B4 AUC recipe disclosure + dungeon fig no-LOCF (Reporting):** manuscript states AUC horizon = min completed real evals across arms/seeds; per-seed linear interp (step 50) + trapezoid/horizon; paired Wilcoxon on per-seed ΔAUC. Appendix B4 anytime figures regenerated without carrying filter curves past support (`export_manuscript_figures.py --fig 9`). Locked Holm numbers unchanged (`v5_maze_statistics.json` / `v4_dungeon_statistics.json`). |
| 2026-08-09 | **Warm-start quantitative disclosure (Reporting):** manuscript/protocol replace ambiguous “≈2614 elites” with JSONL records (2 614) vs collapsed occupied niches (971 / 38.84% coverage; QD ≈417.9); same baseline file all seeds; collapse = max fitness, first on ties; note pyribs `warm_start_elites` = line count. Boundary condition vs terminal hints ≈58–60%. Does **not** amend locked families or rerun arms. |
| 2026-08-09 | **Title + audit checklist formalization (Reporting):** title narrowed to *Auditing Attribution in Surrogate-Assisted MAP-Elites: Target Selection, Prompt Scalars, and Evaluation Gating* (drops “attach points in LLM-guided” framing). Manuscript adds Table `tab:audit-checklist` (8 reusable checks). Does **not** amend locked families or numeric results. |
| 2026-08-09 | **Mixed-stack 2×2 promoted to Results (Reporting):** design → Methods `\S mixed-2x2-design`; table/results → Results `\S mixed-2x2` (was Limitations). Adds paired soft×hard interaction (term −0.11 / cov@20k −0.56 / AUC@20k +0.14 pp); explains soft SDΔ 0.29 vs 1.19 (same-tier re-run vs cross-tier frozen pairing). Descriptive status unchanged. Script `analyze_mixed_2x2.py` emits interaction. |
| 2026-08-09 | **Figure caption cleanup (Reporting):** drop “Memory figure” from Fig.6; dungeon anytime plot titles “Dungeon …” (not “B4 dungeon”); heatmap seed~4 = most negative / largest \|Δ\|; appendix panel ordered 4/6/1 by \|Δ\|; elite fig caption max/50th/10th matches High/Median/Low. Regenerated `--fig 9` and `--fig 7 --panel`. |
| 2026-08-09 | **Figure reporting polish (Reporting):** anytime ladder caption = protocol seeds 0–4 (not outcome-selected); Fig.5 Student-t 95% CI; Fig.4 paired slope/dot plot; dungeon colorblind-safe styles/markers; heatmaps marked illustrative; elites fig = morphology only (edits → §edit-anatomy). Regenerated `--fig 4 --fig 5 --fig 9`. |
| 2026-08-09 | **Editorial / numerical polish (Reporting):** abstract rewritten self-contained (no §/table refs; safe descriptive framing of H1/H2); wall LLM vs genetic ~15× / order of magnitude (was “10×”); mean elite fitness wording; opaque Bound/Mean-TOST labels clarified; $n{\le}3$ pilots + Path A rewrite demoted to appendix; bug ledger table (compose-gate 0.5 vs 0.95 + NMAE denominator) + consistency note that archive endpoints unchanged after hold-out fixes. Numbers and locked families unchanged. |
| 2026-08-09 | **Conditions table rebuild (Reporting):** `tab:conditions` adds separate rows for `stub_uniform` and `hints_minfit`, plus columns for target selection, proposal slots, realized sims, and LLM-call budget so matched H1 / policy×hint are recoverable from the table alone. Numbers and locked families unchanged. |
| 2026-08-09 | **±2 pp margin polish (Reporting):** substantive rationale adds ~3.3% of observed ~60% coverage; explicitly not SD/TOST/pilot-driven; `tab:h1-tost-margin` adds $\delta_{\min}$ (min symmetric band covering 90% CI: qwen 0.95 / gpt 0.72 / DeepSeek 3.06). Numbers and locked families unchanged. |
| 2026-08-09 | **2×2 inferential-status narrative (Reporting):** manuscript clarifies confirmatory bundled stub→hints (Holm) establishes the attribution claim; subsequently completed policy×hint $2\times2$ is a stronger *descriptive* replication (not preregistered factorial/ANOVA; not an upgrade of matched-uniform TOST to confirmatory). Mixed-stack soft×hard $2\times2$ remains descriptive *by design* (locked before first seed), distinct from the policy×hint grid. Numbers and locked families unchanged. |
| 2026-08-09 | **H1 claim-facing rename (Reporting):** Results/Methods labels use *Parent-level surrogate scalars* (not class-wide "Before-generation surrogate"); before-/after-generation kept as architectural roles. Numbers and locked families unchanged. |
| 2026-08-09 | **Main-text demote / RQ framing (Reporting):** Intro frames RQ1–RQ3 (attribution / filter buy / wall identity); H1–H5 kept as audit ledger only. RQ1 Results = decision paragraph + policy×hint table; TOST/±2/providers/QD-cost/Cook → `\S stats-appendix`. RQ2–RQ3 = short decision + identity; ranking/oracle/skip tables retained as supporting. Numbers and locked families unchanged. |
| 2026-08-09 | **Warm-start major boundary (Reporting):** Limitations `\S warm-start-limit` elevates shared floor (2614 JSONL → 971 niches / 38.84%) as major scope bound—mid/late continuation, not cold-start build-from-empty; stage×attach untested. Softens residual “where attach” language; checklist adds warm-start/stage disclosure (now 9 checks). Numbers and locked families unchanged. |
| 2026-08-10 | **Cold-start mixed 2×2 full n=10 authorized (descriptive):** tier `q1-v3-mixed-2x2-cold-smoke`; seed-0 smoke done (hints−stub +9.88 pp term); investigator override to seeds **0–9** (`Q1_COLD_START_SMOKE.md`); primary soft residual + secondary hard/interaction; not Holm/TOST; does not amend warm H1. Parallelism = warm mixed-2x2 (`COLD_SMOKE_WORKERS=2` only; overlapping 5–9 wave killed). |
| 2026-08-13 | **Cold-start mixed 2×2 n=10 complete (descriptive):** 40/40. Soft hints−stub **+1.48±3.57 pp** term (7/10; seed-0 +9.88 was an outlier); @20k +1.07. Hard @20k **sign-flips vs warm** (filter_stub−stub **−1.72** / filter−hints **−2.01**; 3/10 and 2/10) vs warm +3.38 / +2.82 (10/10). Filter skip ~23.5% vs warm ~33.8%. Interaction near-null. Does **not** amend warm H1/H2/Holm or `tab:mixed-2x2`. Manuscript: `\S warm-start-limit` / `\S bootstrap-hyp`. |
| 2026-08-13 | **Path 1 structural rewrite (Reporting):** manuscript title/Intro/Contributions now centre RQ1 attribution (bundled +15 pp → policy×hint → matched residual). H2/gating demoted from co-equal RQ2–RQ3 to a budget-axis illustration (`\S h2`); mixed 2×2 / H3 / H4 / H5 framed as bounds. Checklist presented as instantiated hygiene + failure modes, not a new methodology. **Numbers and locked families unchanged.** |
| 2026-08-13 | **Causal-language hedge on +15 pp (Reporting):** matched-control collapse, not causal mediation (cross-tier 2×2, not interleaved). Genetic `genetic_me`→`genetic_me_uniform` **+13.1 pp** framed as API-free policy bound; dose `target_selection` 50/50 vs scalars ≈10/50; wall 137 vs 93.6 marked throughput not coverage confound. **No new runs.** Numbers and locked families unchanged. |
| 2026-08-13 | **H1 label hygiene (Reporting):** claim-facing H1 is redundant parent-level scalars (true $f$ already in the prompt), not a general before-generation / “weak soft channel” test. Path A reported as bundled two-pass rewrite (no rewrite-stub; ~80% identical drafts; child NMAE shift), not a stronger-actuator failure. **No new runs.** Numbers and locked families unchanged. |
| 2026-08-13 | **H2 break-even formula (Reporting):** wall condition is \(t_{\mathrm{pred}}+\Delta t_{\mathrm{gate}}<s\,\mathbb{E}[T_{\mathrm{sim}}\mid\mathrm{skip}]\) (Eq. h2-breakeven); \(t_{\mathrm{MLP}}<s\,t_{\mathrm{sim}}\) is the constant-\(t\) special case, not an identity. ~7× kept as hardware-specific constant-\(t\) illustration; \(\mathbb{E}[T\mid\mathrm{skip}]\) unmeasured (no per-candidate timers on frozen H2). Training amortized offline. **No new runs.** Numbers and locked families unchanged. |
| 2026-08-13 | **H2 stage floor + nightly buffer overlap (Reporting):** genetic H2 \(+3.65\) pp @20k pinned to warm-start \(971\) niches / \(38.84\%\); mixed-stack empty \(2\times2\) does not isolate stage for `genetic_me_*`. Nightly baseline archive (surrogate off) is the floor file; `buffer_nightly` is backfilled from it, so floor elites sit in the training distribution by construction (covariate overlap, not archive-label leak). **No new runs.** Numbers and locked families unchanged. |
| 2026-08-13 | **Fitness–descriptor coupling (Reporting):** \(w_D{=}0.45\) on BC axis \(D\); `min_fitness_frontier` = lowest-\(f\) occupied frontier cell (not “best”). Warm-start collapsed: Spearman \(\rho(f,D){=}0.986\), \(r{=}0.977\); min-fit cell \(D{=}0.0078\), \(f{=}0\) vs frontier-mean \(D{=}0.459\). Terminal merged stub/stub_uniform \(\rho{\approx}0.985/0.970\). +15 / +13.1 pp *magnitude* is this geometry; attribution (policy vs scalars) holds within it; matched H1 residual less exposed. Offline recompose demoted (cannot recover insertion→parents→proposals); live weight-swap not run. Table `tab:fd-coupling`. **No new QD runs.** Numbers and locked families unchanged. |
| 2026-08-13 | **Hold-out data-lineage one-liner (Reporting):** split = iid 20% row shuffle (`holdout_split`, `random_state=42`); buffer \(N{=}35{,}114\) → \(35{,}087\) unique hashes, 27 extra exact dups, 9 train∩hold-out hashes; all 971 collapsed floor elites in buffer. Exact dups do not explain \(R^2\); clustered SE still not claimed. **No retrain.** Numbers and locked families unchanged. |
| 2026-08-13 | **H2 timeline Kind → descriptive §3.8 (Reporting):** `tab:timeline` H2 row is no longer Kind=Lock (matched genetic H2 has no Holm family). Data mtime 16–17 Jul vs DONE text 17 Jul disclosed as internal-audit overlap. **No numeric change.** |
| 2026-08-13 | **Discrete CMA/pbCMA → main-text H4; drop “unfair CMA” (Reporting):** native discrete CMA-ME **65.57%** / pbCMA **69.95%** / hints **60.41%** / `rint` CMA-ME **73.37%** promoted to Table `tab:discrete-cma`. Claim-facing term is *representation-mismatched end-to-end baseline* (informative as a pipeline; causal emitter-effect limited). Dual-report F-RQ4 FAIL unchanged. Limitations: single CA testbed; no live SAIL/acquisition-management arm (offline GP+UCB ≠ SAIL). **No new runs.** Numbers and locked families unchanged. |
| 2026-08-13 | **Drop unverified API $ from tab:cost (Reporting):** estimated $\sim$1–2 column removed (no token logs on frozen runs). Table keeps measured wall + counted emit attempts. Frozen LLM arms remain integrity-checked, not bit-reproducible (`llm_call_log` not retroactive). Genetic $+13.1$ pp is the API-free bound on the policy collapse. Future work: open-weight pinned checkpoint or interleaved $2\times2$ with call logs—as an upgrade, not a condition of the current claim. **No new runs.** Numbers and locked families unchanged. |
| 2026-08-13 | **Metrics / presentation hygiene (Reporting):** $\pm$ = SD across $n{=}10$ seeds unless a caption names a CI (Fig.5 unpaired $t$ CI on arm means; paired $\Delta$ in Fig.4 / tables). Coverage = occupied / 2500 tessellation cells, not a proven achievable set; CMA-ME occupancy is an empirical reachability bound (no infeasible-cell oracle). Occupied-bin mean fitness changes denominator; QD remains the companion; common-support quality not computed. AUC recipe discloses linear interp of logged series (`np.interp`+trapezoid), not right-continuous step integral; locked Holm unchanged. `tab:mixed-2x2` interaction QD cell is now $\Delta$QD $+1.9\pm26$ (AUC cov@20k stays in text). `tab:h2-ranking-controls` shows `filter_eval_matched` at $49{,}000$ slots / ${\sim}33.2$k evals vs $32.5$k. `tab:cost` adds wall SD (filter $108.5\pm23.8$ min). **No new QD runs.** Numbers and locked families unchanged. |
| 2026-08-13 | **Illustration hygiene (Reporting):** dungeon Fig.B4 is two-panel — crop anytime at AUC horizon **2991** + terminal at **5k proposal slots** (filter early stop = fewer sims, not unfinished). Heatmaps: `[0,1]` Diversity→ / Stability→ axes; gray = empty not infeasible; new n=10 occupancy / $\mathbb{E}[f\mid\mathrm{occ}]$ / $\Delta P$ (`fig07_occupancy_n10`); seeds 4/6/1 remain outcome-selected illustrations. Elites Fig.2: max / 50th / ~10th percentile of filled; bin / BC / genome hash on panel; life/food/boundary legend; 2×2 = binary TH; columns mapped onto Eq. fitness. Regenerated `--fig 9 --fig 7 --panel --fig 2`. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Editorial / terminology hygiene (Reporting):** abstract ≤250 words; Elsevier highlights ≤85 characters; `pp` defined at first use; target selection vs parent glossary + frontier pseudocode; claim-facing drop of equivalence / “full power” / oracle-replay (now true-fitness-threshold replay); Wilcoxon floor printed $2^{-10}=9.77\times10^{-4}$; PPS = expected 0.5 under paired null; DeepSeek LOO shortened to influence note. Dual-report F-RQ4 FAIL retained in audit trail. Frozen `mean_best_fitness` column not renamed. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Volume hygiene (Reporting):** merge duplicate H1–H5 ledger (`tab:h-status` absorbs former `tab:families` note: bundled/floor/multi-provider/dungeon/CVT/checkpoint Holm widths). Canonical H1 mean+CI home = `\S decomposition`; Discussion/Limitations/Conclusion point rather than reprint. Genetic H2 ranking-controls, true-fitness-threshold replay, and mixed-stack 2×2 cell table → appendix `\S h2-controls-appendix`; main keeps decision + Fig.6 + skip/τ. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Floating-alias identification (Reporting):** claim-facing `\S llm-id` ranks bounds already collected—genetic **+13.1 pp** (no API); mixed-2x2 same-tier residual **+0.31±0.29**; `gpt-4o-mini` **+0.12** (response snapshot `gpt-4o-mini-2024-07-18`); cross-tier `q1-full/hints` vs later `stub_uniform` **+0.26** is not bit-reproducible (floating `qwen-turbo`, non-interleaved, no call log). DeepSeek Cook \(D_8\) is not a qwen backend-drift proxy. Interleaved dated-ID `hints` vs `stub_uniform` = upgrade of residual identification, not a condition of the policy-collapse claim. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Maze H5 MDE next to Holm table (Reporting):** ledger Reading is **FAIL (\(2/8\))** (only non-LLM filter AUC rejects), not “partial transfer”. Table `tab:b5-holm` note: retrospective difference-test MDE (Eq. mde, \(t\) not \(z\); before Holm \(m{=}8\)) hints−stub cov AUC **≈2.6 pp** (observed +1.5 pp below MDE); genetic_filter cov AUC **≈2.3 pp** (was ≈2.1 under \(z\)). Non-reject = fail-to-reject, not in-band null; LLM+filter / interaction wrong-sign, not near Holm. **No new QD runs.** Locked Holm unchanged. |
| 2026-08-13 | **Compress H3 appendix diagnosis tables (Reporting):** drop `tab:h3-prod-levels` and `tab:h3-compose-tau`; keep locked gray-zone Holm (`tab:h3-gray-holm`) + dual-compose paragraph + two-sentence \(\tau\times\)emitter replay (\(d{=}0.51\)–\(0.58\) at \(\tau{=}0.45\); \(d{<}0.05\) only at skip \({\gtrsim}87\%\)). `tab:bug-ledger` unchanged. Does **not** hide the 0.5 vs 0.95 bug or rehabilitate production filter. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **QD companion scope + DeepSeek Cook in Discussion (Reporting):** Methods `\S stats` states primary-grid QD-score is a descriptive companion (not Holm); maze/dungeon H5 QD-score AUC remains confirmatory. Discussion `\S h1-impl-threat` names Cook \(D_8{\approx}0.85\); dropping seed 8 does not amend the \(n{=}10\) DeepSeek not-met row or license pooling. Appendix influence note retained. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Results spine + shorten Future work (Reporting):** Results open with load-bearing RQ1 collapse (`\S decomposition` / `tab:policy-x-hint`); later subsections marked bounds/illustrations, not a second claim. Future work drops the completed-package inventory; remaining upgrades are not a condition of the policy-collapse claim. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Route A genre / attribution design table (Reporting):** Genre drops “not a methodology”; keep not-a-new-emitter / not-SOTA / not-portable-ranking. Contribution = empirical attribution of a hybrid EA gain (same stack, two control designs, opposite conclusions: bundled Holm PASS $+15.09$ vs matched residual $+0.26$). Contributions item 3 is the operationalization (checklist), not a hygiene list. Results Table `tab:rq1-design`. Not causal; not a new experimental-design theory. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Elevate warm-start on claim-facing surfaces (Reporting):** title adds *Warm-Started*; abstract/intro/conclusion state the primary result as mid/late continuation ($971$ niches / $38.84\%$), not cold-start illumination. Empty mixed $2\times2$ is not a policy$\times$hint re-run from empty. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Stage-dependent hard gate as central limitation (Reporting):** Contrib 5 / Conclusion / H2 / mixed $2\times2$ lead with mixed-stack cov@20k sign-flip (warm $+$ vs empty $-$); warm H2 must not be generalized to cold-start illumination. Descriptive; genetic-stack H2 untested from empty; not a confirmatory stage×attach factorial. Soft residual stays noisy near-null. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **H1 claim-facing object = mean Δ+CI, not TOST (Reporting):** Contrib 2 / Discussion admissible / `tab:test-map` / Limitations: exploratory paired mean + 90% CI vs policy Δ (~15 pp); not a prospective equivalence test. DeepSeek: CI not near zero, without TOST ritual. TOST / post-hoc ±2 pp stay appendix (`tab:decomposition`). Holm/Wilcoxon unchanged. **No new QD runs.** |
| 2026-08-13 | **Three reader-facing evidence roles (Reporting):** main-text glossary Primary / Robustness / Supplementary (`tab:h-status`); C/E/D/Dual and Lock/Extension/Reporting stay in protocol appendix. H3-prod/gray collapsed to one supplementary row; H4 ceiling = robustness, locked FAIL in appendix. Holm unchanged. **No new QD runs.** |
| 2026-08-13 | **Main-text volume cut (Reporting):** skip/τ (`tab:skip-mech`, `tab:h2-tau-sweep`), pins, test-map, H5 maze levels+AUC recipe, H4 `tab:frq-ceiling`+spatial essay, H1-impl-threat placebo/package A, `tab:fd-coupling` → appendix. Main keeps RQ1 spine, Fig.6, Eq. h2-breakeven, Fig.7, `tab:discrete-cma`, `tab:b5-holm`, `tab:cold-2x2`. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Spine compression (Reporting):** related work → Jin-map + attribution contrast (`tab:positioning` appendix); method surrogate/pyribs → pointers; genetic floor folded into RQ1; H3/multi-LLM one-sentence bounds; threats stop reprinting. Appendix not deleted. H4/H5/cost/`tab:cold-2x2` stay main. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Related-work gap = pipeline attribution (Reporting):** `\S related` now states complete-pipeline LLM+QD comparisons co-vary emitter / policy / warm-start / budget, so a gain need not belong to the claimed channel; demonstration = own bundled $+15.09$ → matched $+0.26$. Not “nobody compared H1 vs H2”; not a field-wide config audit. Jin table = this stack’s topology. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Composite-operator EC motivation (Reporting):** Intro: LLM emitter = composite intervention; fair comparison / budget / initialization as ordinary EC hygiene (not checklist-first). Genre: motivation = confounding; checklist = hygiene, not a new experimental-design theory. `\S related` instantiates. Cites Eiben & Smit 2011, Bartz-Beielstein 2006. Checklist caption maps four rows. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Drop Intro LLM-vs-baseline ranking (Reporting):** practitioner block keeps (i) matched policy and (ii) redundant scalars; drops “use LLM when editable genotypes” + coverage/wall ladder vs genetic ME / CMA-ME. Ladder stays Results H4 / cost and `\S when-llm`. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Compress Results H4 (Reporting):** H4 → one paragraph + `tab:discrete-cma`: not RQ1, not a fair comparison; `rint` ~13 pp expected from encoding; discrete/pbCMA stay because the ceiling survives matched representation. Drop paired CMA-MAE essay and reprint “H4 reading.” Fig.7 stays. Caption: reference ceiling, not ranking. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Sphere H2 in main; dungeon appendix-only (Reporting):** H2: Fontaine Sphere reproduces evaluation-indexed +7.88 pp (10/10; `tab:sphere-h2` in main). Not CA-only; not H1/LLM; not a portable ranking vs CA +3.65. Domain roles: CA primary; maze locked PCG/LLM H5; Sphere literature gate-check; dungeon appendix (repeats maze). Drop dungeon from `tab:conditions`; compress dungeon appendix. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Three-budget conceptual figure (Reporting):** Methods `fig:budget-axes`: slots / real sims / wall with this-stack H2 signs. Caption: incompatible notions of budget in evaluation-skipping QD; illustration, not a ranking. Eq. h2-breakeven prices only wall. Not a new RQ; not in Intro. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **H2 eval-count vs wall as observation (Reporting):** Same skip policy: eval-count win + wall loss on cheap CA is the observation, not a defect. Maze injected-delay δ=10 ms = descriptive regime transition (filter faster 9/10), not a naturally expensive domain. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **H2 phrased as evaluation efficiency, not coverage improvement (Reporting):** abstract / Contrib 4 / `\S h2` / Fig.6: $+3.65$ pp = sample efficiency at matched real evaluations, not a terminal performance gain. Title unchanged (attribution, not H2). **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Surrogate accuracy vs utility (Reporting):** hold-out NMAE = global predictive accuracy, not evolution-control utility (Jin 2005). Layers: NMAE / shadow AUROC / archive gating. Related work + Methods + `\S h2-impl-threat`. Not a new RQ. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Hold-out algebra stays appendix (Reporting):** main: one-liner NMAE=0.112 screen + online-stream ranking. Drop \(R^2\)/\(0.5\) vs \(0.95\) from Methods; Results does not reprint NMAE. H3 compose-gate mismatch stays. Algebra appendix unchanged. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Bug ledger one-liner in main (Reporting):** Contrib 3: two measurement inconsistencies; neither affected archive insertion or primary H1/H2. Names/chronology stay appendix `tab:bug-ledger`. Not H4 category correction. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Bundled Holm = pipeline difference, not scalar causation (Reporting):** Results / `tab:rq1` / `tab:h-status` / `tab:rq1-design` / Discussion: confirmatory for pipeline-level Δ, not prompt-scalar causation. Holm \(m{=}2\) unchanged. **No new QD runs.** |
| 2026-08-13 | **RQ1 observed-contrast table (Reporting):** manuscript `tab:estimands` writes \(\Delta_{\mathrm{bundle}}=Y(\mathrm{h},\mathrm{u})-Y(\mathrm{s},\mathrm{m})\) (off-diagonal; `hints`=uniform, `stub`=minfit), plus matched soft/policy contrasts and the cell-mean identity \(\Delta_{\mathrm{bundle}}=\Delta_{\mathrm{policy}\mid\mathrm{s}}+\Delta_{\mathrm{soft}\mid\mathrm{u}}\). Observed means, not potential outcomes / not mediation. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Results roadmap + checklist to Methods (Reporting):** Results opener groups attribution / evaluation control / robustness (`subsubsection*` only; no new `\section`; H2 not re-elevated). Checklist `tab:audit-checklist` → Methods `\S terminology`. `tab:cost` under `\S h2`. Intro trimmed vs Contributions. No Attribution Framework section. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Cook \(D_8\) out of Results `\S llm-id` (Reporting):** Results ranked bounds drop the statistic; Discussion + appendix keep \(D_8{\approx}0.85\). Not a qwen drift proxy. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Abstract / highlights / wall accounting / confirmatory (Reporting):** abstract cut ~40% (floor + collapse + near-null + not-emitter + gating-as-efficiency + cold sign-flip). Highlights: four science bullets ≤85 chars; warm-start in bullet 2. Eq. h2-breakeven dropped from Contrib 4 and Results opener; subsection retitled wall-clock accounting (formula kept in `\S h2`). Main-text “confirmatory” → Holm / locked / Primary (`tab:h-status`). No confounding diagram. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Contribution = measured attribution failure, not checklist (Reporting):** title *Target Selection Confounds Prompt-Scalar Claims in Warm-Started Surrogate-Assisted MAP-Elites* (drops *Auditing* as head noun). Contrib 3 merged into 1; inversion is the contribution; `tab:audit-checklist` stays Methods, caption not “reusable audit.” Genre / Discussion / Conclusion: reusable lesson = match target selection, not a method. Checklist not moved to appendix. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Related-work novelty = Option B (Reporting):** do not claim prior LLM-QD studies generally suffer this confound; no 5–10-paper config-audit matrix. Related / Intro: complete-pipeline swap *can* co-vary emitter / policy / warm-start / budget; demonstration = own $+15.09$ → $+0.26$. `tab:positioning` remains a mechanism map, not a coding of cited designs. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Causal identification hedge (Reporting):** highlight/RQ1 drop “was a confound” / “attributable to”; genetic $+13.1$ is a bound, not identification of Holm $+15.09$; do not claim policy caused that contrast; admissible reading = target selection accounts for the overwhelming majority of the *observed* bundled gap. `tab:rq1-design` policy rows marked observed, not Holm-Δ ID. Optional abstract / interleaved-upgrade skipped. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **H1 residual = expected leftover, not a general scalar-null (Reporting):** Contrib 2 retitled leftover-after-matching (not standalone primary). Highlight 2 / Intro / Discussion: surprise = bundled misattribution, not the near-null; do not claim prompt scalars do not help LLM emitters. Practitioner: drop “spend on child-level”; Path A stays appendix bound. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Submission PDF split (Reporting):** Genre: main = attribution + named bounds; extra figures / chronology / locked-family tables = Supplementary. `scripts/compile_manuscript.sh` `all` / `split` / `glue` / `both`. Does **not** move H4/H5/H2 out of main. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **Table/figure hygiene: cold-soft star + Fig.5 H4 group + maze vs CA skip-timing (Reporting):** `tab:rq1-design` empty-archive $^\ast$: not distinguishable from zero at $n{=}10$. Fig.5: CMA bars visually grouped as H4 `rint` reference. `\S h2-breakeven`: maze $\delta{=}10$ ms confirms constant-$t$ on maze, does not measure CA $\mathbb{E}[T\mid\mathrm{skip}]$. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **One floor file: seed unit + CMA exclusion + floor snapshot (Reporting):** $n{=}10$ = continuation RNG on one JSONL, not archive-to-archive variance. R/G/L slots share `select_target_cell`; pyribs CMA does not. Visit entropy/repeat unmeasured (no `proposal_log`). Appendix Algorithm + `fig_floor_frontier`. Title unchanged. **No new QD runs.** Holm unchanged. |
| 2026-08-13 | **H2 wall = genetic pair; 20k descriptive; $\tau$ inherited (Reporting):** Claim-facing H2 wall = `genetic_me_filter`−`genetic_me_uniform` only (paired mean $\Delta{+}13.6$ min; ${\sim}3.2\times$; $19.9\pm1.1$ vs $6.3\pm1.0$; 0 LLM). LLM `filter` $108.5\pm23.8$ min is stack cost, not that contrast. cov@20k = descriptive among protocol §3.8 10k/15k/20k (all 10/10), not Holm; AUC@20k $+0.025$ (10/10; no paired AUC CI). Equal-sims $\Rightarrow$ more proposals (MLP on all 32.5k slots); `filter_eval_matched` stays appendix. $\tau{=}0.45$ inherited from v2 combat/shadow skip-band, not tuned on matched genetic H2. **No new QD runs.** Holm unchanged. |
| 2026-08-17 | **Calendar-blocked RQ1 $2\times2$ complete (Extension + Reporting):** tier `q1-rq1-contemporaneous`; 5 independent 971-niche floors × 2 continuations × 5 arms = **50/50**; dated `gpt-4o-mini-2024-07-18`; call logs. New coverage: policy **+14.53 / +14.82 pp** (10/10); leftover **+0.29 pp** @uniform; identical archives at minfit 10/10; shuffle **+0.28 pp** (right sign; mismatch ~89%). Manuscript `tab:rq1-cf` / `\S rq1-cf`. **Not Holm; not TOST; does not identify historical Holm $\Delta$.** Locked families unchanged. |
| 2026-08-17 | **Genetic `max_fitness_frontier` launched (Extension):** tier `q1-v3-genetic-me-maxfit` seeds **0–9** serial; YAML `map_elites_scheduler_nightly_genetic_me_maxfit.yaml`. Descriptive third archive policy on 20R+30G; same warm-start floor. Protocol [`Q1_GENETIC_MAXFIT.md`](Q1_GENETIC_MAXFIT.md). **Does not amend Holm / H2 / RQ1.** |
| 2026-08-17 | **Genetic `max_fitness_frontier` complete (Extension):** 10/10. Terminal cov **50.16 ± 1.36%**; paired vs frozen minfit **+4.23 ± 1.12 pp** (10/10); vs uniform **−8.87 ± 1.44 pp** (0/10). Unique targets **4.4 ± 1.6** / 32 500 (range 2–8). Both extremes below uniform; not a one-sided minfit pathology. [`ANALYSIS.md`](experiments/q1-v3-genetic-me-maxfit/ANALYSIS.md). **Not Holm; does not amend H2 / RQ1 / manuscript.** |
| 2026-08-17 | **RQ1 second-domain maze probe launched (Extension):** Phase A `q1-v5-maze-genetic-minfit` seeds **0–9**; genetic `min_fitness_frontier` vs frozen H5 `genetic` (uniform). Primary cut coverage @ **5k** evals (uniform **57.0±4.5%**). LLM 2×2 gated on GO. Not H5; not Holm. Protocol [`Q1_RQ1_SECOND_DOMAIN.md`](Q1_RQ1_SECOND_DOMAIN.md). |
| 2026-08-17 | **RQ1 second-domain maze Phase A GO (Extension):** 10/10. @ 5k minfit **16.79 ± 3.29%** vs frozen uniform **56.98 ± 4.47%**; Δ **−40.19 ± 4.13 pp** (10/10). Terminal 45.11% vs 99.62% (uniform saturates; minfit does not). [`ANALYSIS.md`](experiments/q1-v5-maze-genetic-minfit/ANALYSIS.md). Phase B LLM 2×2 authorized. **Not Holm; does not amend H5 / CA RQ1.** |
| 2026-08-17 | **RQ1 maze Phase B launched (Extension):** tier `q1-rq1-maze-factorial`; `{stub, live} × {minfit, uniform}`; empty archive; **5k** slots; dated `gpt-4o-mini-2024-07-18`; call logs; seeds **0–9**. Not occupancy-matched to CA floor. Protocol lock in [`Q1_RQ1_SECOND_DOMAIN.md`](Q1_RQ1_SECOND_DOMAIN.md). **Not Holm; not H5 `llm_*` @ 32.5k; does not identify CA Holm Δ.** |
| 2026-08-18 | **RQ1 maze Phase B complete (Extension):** 40/40. Empty-archive coverage @ 5k: policy **+39.10 / +39.31 pp** (10/10); leftover **+0.02 ± 0.14** @ minfit / **+0.23 ± 3.42** @ uniform. Same qualitative inversion as CA; maze pp not portable. [`ANALYSIS.md`](experiments/q1-rq1-maze-factorial/ANALYSIS.md). **Not Holm; not H5; does not identify CA Holm Δ.** |
| 2026-08-18 | **RQ1 maze Phase B into manuscript (Reporting):** main-text bound `tab:rq1-maze` / `\S rq1-maze`; Methods `\S rq1-maze-design`. Not a new primary RQ. Locked Holm H5 / CA RQ1 unchanged. |
| 2026-08-18 | **Title / novelty / causal-wording polish (Reporting):** title *Matching Target Selection Reverses a Bundled LLM-Emitter Advantage in Warm-Started Surrogate-Assisted MAP-Elites* (drops *Confounds* as head verb; keeps *Warm-Started*). Contrib 1 heading: bundled reverse when unmatched. Novelty paragraph: matching is ordinary hygiene; contribution = measured $15\to 0.3$ pp reversal, not a new method. Related: attribution error can be large in one stack (Option B kept). Abstract: $15\to 0.3$; not a new matching method. Claim-facing sweep: mis-attribution → attribution error; keep *confounded* as a table design adjective. **No new QD runs.** Holm unchanged. |
| 2026-08-18 | **Intro / Genre negative-framing cut (Reporting):** Genre rewritten affirmatively + one Scope sentence (warm-started CA continuation; maze = qualitative transfer). Intro keeps one mediation hedge; drops stacked not-emitter / SOTA / ranking / methodology / theorem / Holm–H5 ledger. Novelty no longer reprints Genre. Table / Contrib 4 / Related Option B hedges unchanged. **No new QD runs.** Holm unchanged. |
| 2026-08-18 | **H2 main-text compress (Reporting):** Results `\S h2` = five facts + Fig.6; Sphere table → appendix; Eq. h2-breakeven / AUROC → appendix `\S h2-controls`. `tab:cost` and mixed-stack sign-flip (Contrib 4 / Limitations) stay main. **No new QD runs.** Holm unchanged. |
| 2026-08-18 | **H1 leftover wording (Reporting):** Abstract / Genre / practitioner / highlight 2 drop “parent-level prompt scalars stay near null.” Claim-facing: the tested parent-level scalar augmentation contributed little additional terminal coverage under matched target selection. Calendar-blocked / maze leftovers as $+0.29$ / $+0.02$/$+0.23$ pp. Contrib 2, CI, DeepSeek, not-equivalence unchanged. **No new QD runs.** Holm unchanged. |
| 2026-08-19 | **H1 estimand: providing vs. conditioning (Reporting):** Contrib 2 / `\S h1-impl-threat`: leftover = incremental effect of providing parent-level scalars through this prompt interface, not the effect of a surrogate-derived scalar when the LLM demonstrably conditions on it. Shuffle leftover and no-manipulation-check disclosure unchanged. **No new QD runs.** Holm unchanged. |
| 2026-08-19 | **Policy magnitude vs. misattribution (Reporting):** Abstract / Intro after collapse: policy $+15$ pp *consistent with* this $f$–$D$ geometry; result = bundling with the LLM arm made it liable to misattribution to the prompt channel, not “uniform $>$ min-fitness.” Title, Holm, leftover, maze hedges, `\S fd-coupling` unchanged. **No new QD runs.** |
| 2026-08-19 | **Claim-facing leftover 95% CI; collapsed-archive; sit-below; shuffle $k{=}5$ (Reporting):** Claim-facing leftover = paired $t$ 95% CI (qwen $[-0.59,+1.11]$; `gpt-4o-mini` $[-0.61,+0.85]$; DeepSeek $[-1.49,+3.53]$); `tab:test-map` 95% $t$. TOST 90% appendix-only. Calendar-blocked minfit = matching collapsed archives (occupied bins, elite $f$, stored genotypes). Practitioner: hints 60.4% slightly above genetic 59.0%, not below. Shuffle: clustered bootstrap on $k{=}5$, not confirmatory. **No new QD runs.** Holm unchanged. |
| 2026-08-19 | **Descriptive DiD $\Delta_{\mathrm{int}}$ (Reporting):** `tab:estimands` adds $\Delta_{\mathrm{int}}=\Delta_{\mathrm{soft}\mid\mathrm{u}}-\Delta_{\mathrm{soft}\mid\mathrm{m}}$ ($+0.28$ pp; CI in `tab:policy-x-hint`). Descriptive difference-in-differences for whether the scalar treatment depends on target selection; not mediation of Holm $\Delta_{\mathrm{bundle}}$; not ANOVA; not a new primary. Calendar-blocked $\Delta_{\mathrm{int}}=$ leftover@uniform. **No new QD runs.** Holm unchanged. |
| 2026-08-19 | **Calendar-blocked own subsection (Reporting):** Results `\S rq1-cf` = Independent-floor calendar-blocked $2\times2$ after historical grid; maze empty bound follows as its own subsection. Caption: identification on independent floors (dated ID, call logs), not “just replication.” Not a new Holm primary. **No new QD runs.** Holm unchanged. |
| 2026-08-19 | **Shuffle leftover: small alignment, not almost-null (Reporting):** `\S rq1-cf` / `tab:rq1-cf` note / appendix shuffle: distribution-matched placebo $+0.28$ pp (CI $[+0.02,+0.54]$) = at most a small incremental alignment effect, far below the target-selection effect—not a second ~15 pp channel. Not Holm; not an equivalence claim. Live−stub leftover, abstract, Holm unchanged. **No new QD runs.** |
| 2026-08-19 | **Main-text cut: H4/H5/cost to supplement (Reporting):** Results spine = RQ1 (bundled / historical $2\times2$ / calendar-blocked / maze empty $2\times2$ / genetic $+13.1$). H2 = one paragraph + Fig. acquisition. `tab:cost`, `tab:discrete-cma`, `tab:b5-holm`, `tab:cold-2x2`, `tab:cross-stack` and heatmaps / anytime ladder → appendix. Discussion cut (mechanistic / 4-item decision rule / bootstrap / cold table). **No new QD runs.** Holm unchanged. |
| 2026-08-19 | **Methods/Contrib cut: checklist, conditions, budget figure to supplement (Reporting):** Contrib 1–2 compressed (numbers stay in Results). `tab:audit-checklist`, `tab:conditions`, `tab:domain` and `fig:pipeline` (linewidth), `fig:budget-axes`, `fig:surrogate` → appendix. Mixed $2\times2$ design = three sentences. `\S h1-impl-threat` isolation paragraph compressed. **No new QD runs.** Holm unchanged. |
| 2026-08-19 | **Related/Methods/Discussion cut: jin-map + pipeline to supplement (Reporting):** `tab:jin-map` and `fig:pipeline` → appendix. Related, illuminator loop, protocol, Discussion, Conclusion compressed. Results spine unchanged. **No new QD runs.** Holm unchanged. |
| 2026-08-19 | **$f$–$D$ coupling to Discussion; de-hedge attribution (Reporting):** `\S fd-coupling` = first Discussion critique of policy magnitude (accept; no live $w_D$ swap). Maze $\rho(f,\mathrm{path})\approx 0.89$ stated as not a weak-coupling ablation. Abstract / Intro: unmatched policy *attributed* $+15$ pp, not “liable.” Decomposition: leftover $\le 0.3$ pp in place of “overwhelming majority.” **No new QD runs.** Holm unchanged. |
| 2026-08-19 | **Explicit (a)/(b) split: magnitude vs. leftover (Reporting):** Contrib 1 and `\S fd-coupling` lead: (a) policy $\Delta$ size consistent with $f$–$D$ coupling; (b) small matched leftover not predicted by that coupling—residual of providing parent-level scalars through a prompt that already contains true $f$. Not “expected”; not “explained by” redundancy. No $w_D$ run. **No new QD runs.** Holm unchanged. |
| 2026-08-19 | **Stub first-use; known-weight prompt; by-construction magnitude (Reporting):** Intro defines historical `stub` at first use (constant placeholders $0.5$/$1.0$ vs. live parent-level scalars). `\S fd-coupling` / `\S h1-impl-threat`: system prompt states Eq. fitness including $w_D{=}0.45$; leftover near-null under both policies bounds that known-weight channel as well as the provided scalars. Magnitude critique valid *by construction*, not “we accept the critique.” **No new QD runs.** Holm unchanged. |

| 2026-08-24 | **Manuscript prose pass, 19–24 Aug (Reporting):** Claim-facing and appendix wording only (abstract/intro/methods/results/discussion; appendices A–C, G, J, K, N). Wording-only edits are not protocol amendments. Notable freeze-ledger wording (now repo-only): internal freeze vs OSF preregistration; dungeon v4 Lock vs matched H2 Reporting; H4 = Lock plus later Reporting; mtime does not establish first inspection. Stats-narrative canon in §4.5 / Appendix A: confirmatory = Wilcoxon+Holm within each locked family; non-Holm estimates (matched leftover, calendar-blocked, maze empty 2×2) = paired mean Δ + 95% CI (seed or floor unit); leftover CI vs policy Δ (~15 pp); ±2 pp provenance = internal extend-or-stop gate, not a locked SESOI. TOST / PPS / MDE remain diagnostic companions. **Appendix Q (freeze/timeline tables) moved out of the journal PDF** to `artifacts/PROTOCOL_FREEZE_TIMELINE.md` + README pointer; Appendix K unchanged. **Abstract/Intro:** claim narrowed to this warm-started CA; maze = own second evaluator; Sphere = H2 only. Abstract split into thesis / numbers / cheap-CA gating; dropped *readily*, *warm-started stack*, *cost-accounting illustration*. **No new QD runs.** Holm unchanged. |
| 2026-08-26 | **Sphere H1 second domain wired + Phase A GO (Extension):** Fontaine Sphere empty-start MAP-Elites; H1 leftover = live parent-level scalars vs stub $0.5$/$1.0$ at matched policy; LLM returns 20 local deltas; dated `gpt-4o-mini-2024-07-18`. Phase A `q1-sphere-genetic-policy` **20/20**: minfit − uniform **−5.79 ± 0.55 pp** (10/10) @ 5k → **GO** Phase B. Protocol [`Q1_RQ1_SPHERE_DOMAIN.md`](Q1_RQ1_SPHERE_DOMAIN.md). Phase B `q1-rq1-sphere-factorial` **not launched**. Does **not** amend Holm, CA H1, maze 2×2, or Sphere H2. Manuscript stays Sphere-H2-only until the LLM grid is read. |
| 2026-08-26 | **Sphere H1 Phase B launched (Extension):** `q1-rq1-sphere-factorial` 2 workers after live preflight **20/20** parse 1.0 / fallback 0 / mean L2 0.68. Dated `gpt-4o-mini-2024-07-18`. Grid **running**, not read. Does **not** amend Holm, CA H1, maze 2×2, or Sphere H2. Manuscript stays Sphere-H2-only until the LLM grid is read. |
| 2026-08-26 | **Main-text “X is not Y” wall → §6.5 (Reporting):** Self-definitions collected into one Limitations paragraph; gating named once in Intro as illustrative. Wording-only. **No new QD runs.** Holm unchanged. |
| 2026-08-26 | **Stacked hedges unstacked (Reporting):** One qualifier per thesis, at paragraph end or in a separate sentence. Maze empty $2\times2$ no longer restates second-evaluator / not-portable / not-primary in the same sentence as the numbers. Limitations wall in §6.5 unchanged. Wording-only. **No new QD runs.** Holm unchanged. |
| 2026-08-26 | **H2 / Holm disclaimers once in §4.5 (Reporting):** H2 = budget-axis illustration, not a co-equal RQ; leftover / calendar-blocked / maze empty / H2 = not Holm decisions. Said once in `\S stats`. Pointers elsewhere; no restatement in Intro, H2 headings, Sphere caption, or appendix H2 figures. Wording-only. **No new QD runs.** Holm unchanged. |
| 2026-08-26 | **Authorial voice in Intro / Discussion (Reporting):** Intro: “We expected the scalars to matter. They did not.” Discussion synthesis: bundled gap looks impressive / evaporates under matched policy / scalars were never doing the work. Maze closer rewritten the same way. Wording-only. **No new QD runs.** Holm unchanged. |
| 2026-08-26 | **Main-text cross-refs thinned (Reporting):** At most one section/table/figure pointer per paragraph in the journal main text. Appendix maps (Jin, checklist, pins, cost, Sphere table, pipeline figure) no longer cited from the running argument. Wording-only. **No new QD runs.** Holm unchanged. |
| 2026-08-27 | **Sphere H1 Phase B readout (Extension+Reporting):** `q1-rq1-sphere-factorial` **40/40** @ 5k empty $100\times100$, dated `gpt-4o-mini-2024-07-18`, call logs, parse $\ge 99.99\%$, fallback $\le 0.01\%$. Policy (uniform − minfit) **$+2.68$ / $+2.47$ pp** (10/10). H1 leftover (live − stub) **$+0.00$ pp** @ minfit; **$-0.21$ pp** @ uniform (1/9; paired $t$ 95% CI $[-0.33,-0.09]$). Same qualitative split as CA/maze; magnitudes not portable (genetic Phase A was $-5.79$ pp). Does **not** amend Holm, CA H1, maze 2×2, or Sphere H2. **Not a new Holm family.** [`Q1_RQ1_SPHERE_DOMAIN.md`](Q1_RQ1_SPHERE_DOMAIN.md); [`experiments/q1-rq1-sphere-factorial/ANALYSIS.md`](experiments/q1-rq1-sphere-factorial/ANALYSIS.md). |
| 2026-08-27 | **Sphere H1 stays out of the journal PDF (Reporting):** Grid is read; numbers stay in the protocol / `ANALYSIS.md`. Journal Sphere remains H2 gating only. Maze empty $2\times2$ remains the second-evaluator attribution table. Reason: compressed policy $\Delta$ (${\approx}{+}2.5$ pp) and leftover @ uniform $-0.21$ pp (CI excludes 0) would weaken the load-bearing $+15$/$+39$ story more than a literature-domain H1 table would help. **No manuscript edits.** Holm unchanged. |
| 2026-08-27 | **Sphere H1 appendix-only (Reporting):** Reverses the same-day keep-out. One appendix table (`tab:sphere-h1`) plus three sentences; not main text, not Holm, not a co-equal domain. Policy $+2.68$/$+2.47$ pp; leftover $+0.00$/$-0.21$ pp (uniform CI $[-0.33,-0.09]$). Maze empty $2\times2$ remains the second-evaluator table. Limitations wall: Sphere leftover is appendix-only. Holm unchanged. |
| 2026-08-27 | **Supplementary Material split (Reporting):** Journal article keeps main text + **six** appendices (A statistics, B checkpoint, C Sphere transfer, D dungeon/maze H5, E evaluator, F H2 controls). Former appendices C, D, E, K, P, Q and the other “not required” blocks (encoding, H3, H1 pilots/prompt, labels, per-seed, heatmap) move to a **separate** Supplementary Material file (`supplementary_body.tex`; S1–S12), not lettered as article appendices. `compile_manuscript.sh` splits at the Supplementary heading (article PDF = main+A–F; `draft_v0_supplement.pdf` = e-component). **No new QD runs.** Holm unchanged. |
| 2026-08-27 | **Citation audit (Reporting):** Related-work cites remapped to what the papers actually do. SAIL no longer cited for “MAP-Elites evaluates every candidate”; EGO is infill ancestry not a SAEA; DMS dropped from the SAIL/DSA-ME acquisition list; Eureka / LLaMEA-behaviour-space dropped from the QD-emitter cluster; Flageat UQ cited as the re-evaluation literature this canonical-seed protocol does not need; LAEA bib → *Swarm and Evolutionary Computation* 91:101741; Lim → ALIFE 2024 DOI; Jin labels = individual-/generation-based; PPS = matched-pair CL (McGraw), not Kerby rank-biserial. **No new QD runs.** Holm unchanged. |
| 2026-08-27 | **Zenodo artifact pin v2 (Reporting):** Manuscript + freeze ledger point at **10.5281/zenodo.22119553** (file `LifeManifold-zenodo-20260827T010428Z-a714e548cfd6.tar.gz`; git tag `journal-v1.1` / `a714e548cfd682645af38a7dd343fe8da26b6e3d`). Supersedes v1 **10.5281/zenodo.21727011** (`journal-v1`) as the current artifact DOI of the same concept. Includes Sphere H1 leftover and maze empty RQ1. **No new QD runs.** Holm unchanged. |

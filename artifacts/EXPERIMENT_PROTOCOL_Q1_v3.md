...
**Manuscript reporting (2026-08-07; category correction, not demotion):** main-text H4 is a **descriptive continuous-relaxation ceiling**, because confirmatory “hints > CMA” was a **category error** given the encoding asymmetry already disclosed at freeze (CMA on $\mathbb{R}^{21}$ + `rint` vs discrete LLM/genetic edits). **Dual-report:** locked F-RQ4 (Holm **m=4**) remains **FAIL** on unchanged numbers; omitting the four Wilcoxon $p$-values from the paper-wide confirmatory vector does **not** erase that FAIL, and restoring them would not convert F-RQ4 to PASS or flip other claim-driving reject/non-reject labels. See §12 row 2026-08-07.
...
| 2026-08-07 | **H4 / F-RQ4 manuscript category correction (Reporting):** **was:** H4 listed as confirmatory Holm family **m=4** (F-RQ4) and included among paper-wide confirmatory $p$-values. **now:** main-text H4 reported as a **descriptive continuous-relaxation ceiling** because confirmatory “hints > both CMA arms” is a **methodological category error** under the encoding asymmetry disclosed at the 2026-07-12 freeze (CMA continuous $\mathbb{R}^{21}$ + `rint` vs discrete LLM/genetic genomes)—not a result-driven demotion after FAIL. **Dual-report (numbers unchanged):** locked F-RQ4 remains **FAIL** (only `RQ4_mae_fit` Holm-rejects; CMA-ME Δcov mean **−12.96** pp); restoring the four Wilcoxon $p$-values to the paper-wide vector would not convert F-RQ4 to PASS or flip other claim-driving reject/non-reject labels. Frozen hypothesis text in §3.2 / §4.1 unchanged; artifact `q1-v3-pyribs/frq4_statistics.json` unchanged. |
| 2026-08-09 | **H1 policy×hint missing cell (descriptive):** launch tier `q1-h1-policy-x-hint` = live hints @ `min_fitness_frontier` (`hints_minfit`, $n{=}10$). Completes the $2\times2$ with frozen `q1-full/stub`, `q1-stub-uniform-sensitivity/stub_uniform`, `q1-full/hints`. Protocol [`Q1_H1_POLICY_X_HINT.md`](Q1_H1_POLICY_X_HINT.md). Does **not** amend confirmatory H1/Holm; softens manuscript causal wording to stub-side recovery until fourth cell is reported. |

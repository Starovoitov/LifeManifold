"""Neural latent → ``WorldSpec`` generator driven by YAML (``neural_world_generator.spec``)."""
...
def load_neural_generator_yaml(path: str | Path) -> dict[str, Any]:
    ...
    if not src.is_file():
        raise FileNotFoundError(
            ...
            "neural_world_generator.spec is installed next to neural_world.py "
...
class NeuralWorldGenerator(WorldGenerator):
    ...
    Default YAML path: ``neural_world_generator.spec`` next to this module.
...
_DEFAULT_SPEC_PATH = Path(__file__).with_name("neural_world_generator.spec")

...
def create_diagnostic_dashboard(
    ...
    *,
    ...
    title: str = "Diagnostic Dashboard",
) -> go.Figure:
    """Composite diagnostic figure (full world panels ship in a later epic)."""
    fig = make_subplots(
        rows=1,
        cols=2,
        column_widths=[0.6, 0.4],
        specs=[[{"type": "xy"}, {"type": "polar"}]],
        subplot_titles=("World (E3)", "Metrics radar"),
    )
    fig.add_annotation(
        text="Full world simulation panels ship in the next epic.",
        xref="paper",
        yref="paper",
        x=0.3,
        y=0.5,
    ...
    if result is not None and result.metrics is not None:
        metrics = {key: float(value) for key, value in asdict(result.metrics).items()}
        radar = create_metrics_radar(metrics)
    ...
    if surrogate_pred:
        ...
        if fitness is not None:
            text = f"Surrogate fitness: {float(fitness):.3f}"
            if uncertainty is not None:
                text += f" (±{float(uncertainty):.3f})"
            lines.append(text)

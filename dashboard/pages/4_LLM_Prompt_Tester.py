...
def _cached_grid_archive(path_str: str, mtime: float, resolution: int) -> GridArchive:
    del mtime
    return load_grid_archive(Path(path_str), resolution=resolution)
...
grid_archive: GridArchive | None = None
selected_path: Path | None = None
chosen_bin: tuple[int, int] | None = None
...
if grid_archive is not None:
    bins = occupied_bins(grid_archive)
    if not bins:
        st.warning("Archive has no occupied cells.")
    else:

        def _bin_label(bin_xy: tuple[int, int]) -> str:
            elite = grid_archive.get(bin_xy[0], bin_xy[1])
            fitness = elite.fitness if elite is not None else float("nan")
            return f"({bin_xy[0]}, {bin_xy[1]}) · fitness={fitness:.4f}"

        chosen_bin = st.selectbox("Target bin", bins, format_func=_bin_label)
    elite = (
        grid_archive.get(chosen_bin[0], chosen_bin[1])
...
if grid_archive is not None and chosen_bin is not None:
    target = target_bin_for_cell(
        grid_archive,
        chosen_bin,
    ...
    rendered_user, format_error = render_user_prompt_preview(user_template, kwargs)
...
system_text = render_system_prompt_preview(grid_resolution, cfg)

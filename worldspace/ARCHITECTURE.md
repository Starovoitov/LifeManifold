...
LLM prompts: `prompts/map_elites_llm_emitter_system.txt` with `{N}` / `{bin_width}` filled by `render_system_prompt(grid_resolution)`; `system_prompt_version()` is the first 8 hex digits of the file SHA-256. User prompts via `build_user_prompt` in `llm_emitter.py` (target niche, surrogate stub, current elite JSON, Moore few-shot from the archive). Shared field bounds: `worldspace/specs/world_spec_constraints.py` (also used by legacy `llm_patch`).

...
Nightly: default pipeline in `worldspace/scripts/run_map_elites_nightly.py` (`make nightly-map-elites`): (1) baseline with `map_elites_scheduler_nightly.yaml` (`iterations: 100000`, surrogate off, buffer `artifacts/surrogate/buffer_nightly.jsonl`) → (2) `scripts/train_surrogate.py` → `artifacts/surrogate/checkpoints/nightly.pkl` → (3) surrogate phase with `map_elites_scheduler_nightly_surrogate.yaml`, resuming baseline archive. Outputs under `artifacts/map_elites_nightly/baseline/` and `.../surrogate/` plus `nightly_pipeline_summary.json`. `--single-run` runs one phase; production scheduler remains `map_elites_scheduler.yaml` (`iterations: 10000`).

...
LLM prompts: `prompts/map_elites_llm_emitter_system.txt` with `{N}` / `{bin_width}` filled by `render_system_prompt(grid_resolution)`; `system_prompt_version()` is the first 8 hex digits of the file SHA-256. User prompts via `build_user_prompt` (target niche, `resolve_surrogate_stub` values from scheduler YAML, current elite JSON, Moore few-shot). Parsing: `worldspace/specs/world_spec_from_llm.py`. Shared field bounds: `worldspace/specs/world_spec_constraints.py` (also used by legacy `llm_patch`).

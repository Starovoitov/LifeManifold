"""Validated, canonical 16×16 dungeon representation."""
...
DOOR = "D"
HAZARD = "H"
TILES = frozenset((WALL, FLOOR, START, GOAL, KEY, DOOR, HAZARD))
...
class DungeonSpec(BaseModel):
    ...
    def _validate_layout(self) -> DungeonSpec:
        if len(self.rows) != self.SIZE:
            raise ValueError(f"dungeon must have {self.SIZE} rows")
        if any(len(row) != self.SIZE for row in self.rows):
            raise ValueError(f"every dungeon row must have length {self.SIZE}")
        unknown = set("".join(self.rows)) - TILES
        if unknown:
            raise ValueError(f"unknown dungeon tiles: {sorted(unknown)}")
        ...
        if counts[START] != 1 or counts[GOAL] != 1:
            raise ValueError("dungeon must contain exactly one start and one goal")
        if counts[KEY] != counts[DOOR] or counts[KEY] > 1:
            raise ValueError("dungeon must contain either zero or one key-door pair")
        return self

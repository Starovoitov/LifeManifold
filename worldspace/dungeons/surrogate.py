"""Dungeon-specific deterministic MLP ensemble and checkpoint utilities."""
...
from worldspace.dungeons.evaluation import DungeonEvaluation
...
@dataclass
class DungeonSurrogateCheckpoint:
...
class DungeonSurrogate:
    def __init__(self, checkpoint: DungeonSurrogateCheckpoint) -> None:
        if checkpoint.feature_names != FEATURE_NAMES:
            raise ValueError("dungeon surrogate feature schema mismatch")
        self.checkpoint = checkpoint

    @classmethod
    def load(cls, path: Path) -> DungeonSurrogate:
        with path.open("rb") as handle:
            checkpoint = pickle.load(handle)  # noqa: S301
        if not isinstance(checkpoint, DungeonSurrogateCheckpoint):
            raise TypeError("invalid dungeon surrogate checkpoint")
...
def buffer_row(
    spec: DungeonSpec,
    evaluation: DungeonEvaluation,
    *,
    design_seed: int,
) -> dict[str, object]:
    return {
        "schema_version": "dungeon-surrogate-1.0",
...
def load_buffer(
    ...
    if len(features) < 20:
        raise ValueError("dungeon surrogate buffer requires at least 20 rows")
...
def train_checkpoint(
    ...
    *,
    ...
    ensemble_size: int = 5,
    max_iter: int = 300,
) -> tuple[DungeonSurrogateCheckpoint, dict[str, object]]:
    ...
    checkpoint = DungeonSurrogateCheckpoint(
...
def save_checkpoint(checkpoint: DungeonSurrogateCheckpoint, path: Path) -> None:

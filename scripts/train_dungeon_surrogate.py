#!/usr/bin/env python3
"""Collect reserved dungeon design data and train its surrogate ensemble."""
...
from worldspace.dungeons.genetics import mutate_dungeon, random_dungeon
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--buffer",
        type=Path,
        default=Path("artifacts/dungeons/surrogate/buffer.jsonl"),
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
    ...
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("artifacts/dungeons/surrogate/report.json"),
    )
    parser.add_argument("--collect-count", type=int, default=2000)
    parser.add_argument("--design-seed", type=int, default=10000)
...
def collect_buffer(path: Path, *, count: int, seed: int) -> None:
    ...
    seen: set[str] = set()
    current = random_dungeon(rng)
    with path.open("w", encoding="utf-8") as handle:
        while len(seen) < count:
            if len(seen) % 3 == 0:
                current = random_dungeon(rng)
            else:
                current = mutate_dungeon(current, rng, edits=6)
            candidate_hash = current.candidate_hash()
            if candidate_hash in seen:
                continue
            seen.add(candidate_hash)
            design_seed = seed + len(seen)
            evaluation = evaluate_dungeon(current, seed=design_seed)
            handle.write(
                json.dumps(
                    buffer_row(
                        current,
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not report["quality_gate_pass"] or not report["shadow_skip_gate_pass"]:
        raise SystemExit("Dungeon surrogate gate failed")

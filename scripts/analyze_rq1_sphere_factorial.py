#!/usr/bin/env python3
"""Readout for Sphere RQ1 Phase B: stub/hints × minfit/uniform at 5k.

H1 leftover = live − stub at matched policy. Descriptive only.
Not Holm, not CA H1, not maze 2×2. Protocol: artifacts/Q1_RQ1_SPHERE_DOMAIN.md
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--root",
        type=Path,
        default=Path("artifacts/experiments/q1-rq1-sphere-factorial"),
    )
    ...
    for arm in ARMS:
        ...
        fallbacks = [
            float((p.get("llm_audit") or {}).get("fallback_rate", 0.0))
            for p in rows.values()
        ]
        parses = [
            float((p.get("llm_audit") or {}).get("parse_success_rate", 0.0))
            for p in rows.values()
        ]
        if fallbacks:
            print(
                f"  fallback={_fmt([100.0 * x for x in fallbacks])}%"
                f"  parse={_fmt([100.0 * x for x in parses])}%"
            )
        if fallbacks and mean(fallbacks) > 0.10:
            print(
                "  WARNING mean fallback_rate "
                f"{mean(fallbacks):.3f} > 0.10 — leftover is contaminated "
                "(both arms become genetic Gaussian)"
            )
        if parses and mean(parses) < 0.90:
            print(
                "  WARNING mean parse_success_rate "
                f"{mean(parses):.3f} < 0.90 — do not read leftover as scalars"
            )
    ...
    contrasts = (
        ...
        ("leftover @ minfit (H1)", "llm_stub_minfit", "llm_hints_minfit"),
        ("leftover @ uniform (H1)", "llm_stub_uniform", "llm_hints_uniform"),
    )
